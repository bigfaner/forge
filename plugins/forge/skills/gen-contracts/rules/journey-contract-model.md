...
3. **Test files**: Generated by gen-test-scripts with surface-key adaptive output paths:
   - **Single surface**: `tests/<journey>/` — no surface-key directory layer
   - **Multi surface**: `tests/<surfaceKey>/<journey>/` — surface-key partitions test files by user-defined identifier
...
| Process | Description |
|---------|-------------|
| Tests generated directly to `tests/<journey>/` (single surface) or `tests/<surfaceKey>/<journey>/` (multi surface) | No staging directory |
