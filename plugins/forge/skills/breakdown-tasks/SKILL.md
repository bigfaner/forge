...
When all tasks are `type: "documentation"` (non-compilable output), skip **Step 0** (language) and **Step 4b** (standard test tasks). **Step 5** (`forge task index`) is always mandatory — without `index.json`, `forge task claim` fails.

**Detection**: Step 1 scans all input artifacts → if every design element targets non-compilable files only, the feature is docs-only.

```mermaid
graph LR
    S1["Step 1"] --> D{"docs-only?"}
    D -->|"skip 0, 4b"| B["Steps 2–3–4a"]
    D -->|No| A["Steps 0→2→3→4a→4b"]
    B --> S5
    A --> S5
...
| `coding.enhancement` | Task improves existing behavior (performance, UX, edge-case handling) without adding new capabilities |
...
Unrecognized or ambiguous tasks fall back to `coding.feature`.
...
2. If `intent` is set and matches a valid type constant (`coding.feature`, `coding.enhancement`, `coding.cleanup`, `coding.refactor`) → use it as the default `type` for all business tasks
...
Test tasks are auto-generated by `forge task index` based on the languages resolved in Step 0. **Do NOT create test task `.md` files manually.** The CLI handles the full test pipeline (T-test-1 through T-test-4.5), per-language expansion, and fix-task generation. Additional tasks (T-specs-1 for spec consolidation, T-clean-code-1 for code cleanup) are generated based on the `auto` configuration block in `.forge/config.yaml`.

**T-specs-1 (Consolidate Specs + Drift Detection)**: T-specs-1 covers the full spec lifecycle pipeline — extract business rules and conventions from feature docs → integrate into project-level specs → detect drift between specs and current code → auto-fix drifted rules. This task validates that existing `docs/conventions/` and `docs/business-rules/` files remain consistent with the actual codebase after each feature iteration. Generated when `auto.consolidateSpecs.full` is `true` (default).
...
3. Auto-generates test task `.md` files from embedded language strategies
...
- [ ] `index.json` ends with test tasks matching the language expansion from Step 0 (auto-generated by `forge task index`)
