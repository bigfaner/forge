...
> **Note**: Test tasks (T-test-*) are auto-generated by `forge task index` in Step 5. They are NOT created manually in this step. CLI handles test pipeline tasks, expansion, and fix-tasks.

Fix-task override (for reference, used by executors during test runs):

**Fix-Type Derivation**: extract `TASK_CATEGORY` from claim output and derive the fix type:
