...
**T-test-5 expanded scope (Consolidate Specs + Drift Detection)**: T-test-5 covers the full spec lifecycle pipeline — extract business rules and conventions from feature docs → integrate into project-level specs → detect drift between specs and current code → auto-fix drifted rules. This means T-test-5 is no longer spec-extraction-only; it validates that existing `docs/conventions/` and `docs/business-rules/` files remain consistent with the actual codebase after each feature iteration.
