...
When all tasks are `type: "documentation"` (non-compilable output), skip **Step 0** (profile) and **Step 4b** (standard test tasks). **Step 5** (`forge task index`) is always mandatory — without `index.json`, `forge task claim` fails.
