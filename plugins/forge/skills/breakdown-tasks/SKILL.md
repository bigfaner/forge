...
Business tasks MUST include `type` in frontmatter — `forge task index` returns a hard error when it's missing. Auto-generated tasks (gates, summaries, fix-tasks, test pipeline) have types inferred from their ID patterns:
...
| No match (business task) | Hard error — must set `type` explicitly |

Available business task types: `"implementation"`, `"documentation"`.
