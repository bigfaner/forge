...
- Phase summary (`<phase>.summary`) and gate (`<phase>.gate`) files are **auto-generated by `forge task index`** — do NOT create them manually.
- Auto-generation triggers for phases with >=2 business tasks (excluding test tasks).
- See Step 4b and Step 4c for details.
...
3. Write `scope` into the implementation task `.md` frontmatter's `scope` field.
...
| Any Affected File participates in compile/build/runtime | `templates/task.md` (type: `"implementation"`) |

```
# documentation — non-compilable, non-runnable
*.md (design docs, specs, templates, rubrics)

# implementation — participates in compile/build/runtime
Source code (*.go, *.ts, *.js, *.py, etc.)
Build configs (justfile, Makefile, Dockerfile)
Runtime configs (*.yaml, *.toml, etc.)
```

Auto-generated tasks (gates, summaries, fix-tasks, test pipeline) have types inferred from their ID patterns:
...
| Task ID is `T-eval-doc` | `"doc-evaluation"` |

### 4b. Phase Summary Tasks (auto-generated by `forge task index`)

Phase summary files (`<N>.summary.md`) are **auto-generated by `forge task index`** in Step 5. Do NOT create them manually. The CLI detects numbered phases from business task IDs and generates:

- `<N>.summary.md` — depends on all business tasks in phase N
- `<N>.gate.md` — depends on `<N>.summary`, `breaking: true`

Auto-generation applies to phases with **>=2 business tasks** (excluding test tasks). Single-task phases and test-only phases are skipped.

To ensure correct auto-generation, business tasks MUST use `<phase>.<sub>` ID format (e.g., `1.1`, `2.3`) so the CLI can detect phase groupings.

### 4c. Gate Tasks (auto-generated by `forge task index`)

Gate files (`<N>.gate.md`) are **auto-generated by `forge task index`** alongside summary files. Do NOT create them manually.

The CLI generates gates for every phase with >=2 business tasks. Example auto-generated dependency chain:

```
Phase 1: 1.1, 1.2                 (dependencies: none or earlier phases)
Phase 1 summary: 1.summary         (dependencies: ["1.x"])  ← auto-generated
Phase 1 gate: 1.gate               (dependencies: ["1.summary"])  ← auto-generated
Phase 2: 2.1, 2.2                  (dependencies: ["1.gate"])
Phase 2 summary: 2.summary         (dependencies: ["2.x"])  ← auto-generated
Phase 2 gate: 2.gate               (dependencies: ["2.summary"])  ← auto-generated
```

Idempotent: re-running `forge task index` preserves existing `.summary` and `.gate` files.
...
2. Auto-generates stage-gate files (`<N>.summary.md` and `<N>.gate.md`) for phases with >=2 business tasks (see Steps 4b/4c)
3. Auto-generates test task `.md` files from embedded profiles (Step 4d tasks)
...
- [ ] Stage-gate files (`.summary.md`, `.gate.md`) auto-generated by `forge task index` for phases with >=2 business tasks
- [ ] Every PRD AC covered by ≥1 task
- [ ] Dependency graph is a DAG (no cycles) — verify with `forge task validate-index`
- [ ] Every Phase Inventory gate has a corresponding gate task (auto-generated by `forge task index`)
