...
**Detection**: During Step 1 (Read All Documents), after scanning all input artifacts — if every design element targets non-compilable files (`.md`, `.yaml`, `.json` config only) and the tech-design or PRD confirms no runtime changes, the feature is docs-only. Skip Step 0 immediately.

```mermaid
graph LR
    S1["Step 1: Read Documents"] --> D{"all design elements<br>non-compilable?"}
    D -->|Yes| S2["Step 2 → 3 → 4a<br><i>skip Step 0 & 4b</i>"]
    D -->|No| S0["Step 0: Resolve Profile"]
    S0 --> S2x["Step 2 → 3 → 4a → 4b"]
    S2 --> S5["Step 5 → 7"]
    S2x --> S5
```

**Workflow for docs-only features**: Step 1 (detect docs-only from documents, skip Step 0) → Step 2 → Step 3 → Step 4a (all tasks use `templates/task-doc.md`, skip Step 4b) → Step 5 → Step 6 → Step 7.
