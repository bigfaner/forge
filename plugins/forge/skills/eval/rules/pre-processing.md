...
| `contract` | Detect surface type from `.forge/config.yaml` `surfaces` field. Load the corresponding surface rule file from gen-journeys skill's `rules/surface-<type>.md` (resolve relative to the gen-journeys skill directory) as additional context for the scorer. |
