...
| `journey` | Detect surface type from `.forge/config.yaml` `surface` field. Load the corresponding surface rule file from gen-journeys skill's `rules/surface-<type>.md` (resolve relative to the gen-journeys skill directory) as additional context for the scorer. |
| `contract` | Detect surface type from `.forge/config.yaml` `surface` field. Load the corresponding surface rule file from gen-journeys skill's `rules/surface-<type>.md` (resolve relative to the gen-journeys skill directory) as additional context for the scorer. |
