...
The api-test-cases.md must contain these sections:

- [ ] Frontmatter with `feature`, `sources`, `generated`
- [ ] API Test Cases section with individual test cases
...
| Criterion | Points | What to check |
|-----------|--------|---------------|
| Steps are concrete actions | 0-90 | Each step describes a single, unambiguous API call. "GET /api/users?page=1" not "Fetch users". "POST /api/users with body `{name: 'test'}`" not "Create a user" |
| Expected results are verifiable | 0-90 | Every expected result can be objectively verified: specific HTTP status code, response body fields, header values. Not "should succeed" or "returns data" |
| Preconditions are explicit | 0-70 | TCs with dependencies (authenticated user, existing resources, specific state) declare them in Pre-conditions. No implicit assumptions |
...
| Integration scenarios | 0-70 | TCs cover cross-feature or cross-interface scenarios (e.g., API call triggers background job, webhook delivery) where applicable and mentioned in the PRD |
| API coverage breadth | 0-60 | All API endpoints described in the PRD have corresponding test cases. Every endpoint, method, and resource mentioned in the PRD is tested |
...
| Classification is correct | 0-30 | Each TC is classified as API type. No UI, CLI, TUI, or Mobile TCs in this file |
...
| Criterion | Points | What to check |
|-----------|--------|---------------|
| Pre-conditions are concrete and creatable | 0-30 | Score only pre-conditions that exist in the document — missing pre-conditions are penalized under D2 (Step Actionability). For pre-conditions that ARE listed, every one must describe HOW to create the required state (e.g., "seed database with 5 test users via POST /api/users/batch"), not just assert it exists (e.g., "users exist"). If a pre-condition cannot be created using an isolated test fixture (temp directory, test container, mock server, etc.), the downstream script will generate a conditional skip without fixture. Deduct 10 pts per non-creatable pre-condition |
| Steps describe runtime behavior | 0-25 | No step describes reading source files (`.md`, `.go`, `.json`), checking documentation content, or verifying file existence. Every step interacts with the running API (HTTP request, webhook call). Deduct 10 pts per static-file-check step |
...
- **Vague language without specificity**: -20 pts per instance ("call endpoint" without method/path, "check response" without expected fields)
- **Cross-section inconsistency**: -30 pts per conflict (e.g., traceability table says TC-005 is UI but it's listed under API)
