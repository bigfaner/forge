...
When multiple capabilities are active, divide 150 pts equally across the remaining (non-excluded) active capabilities. Each capability's sub-criteria use percentage-based weights (not fixed points) to allow clean rescaling:
...
**Scoring example**: If the active capabilities are `api` and `cli` (both with TCs present), each gets 75 pts. For `api`: Contract accuracy scores X/100 × 50% = X × 0.375 of the 75-pt allocation; Error contract coverage scores Y/100 × 50% = Y × 0.375. Total for `api` = X × 0.375 + Y × 0.375 (max 75).

### 4. Completeness (200 pts)

| Criterion | Points | What to check |
|-----------|--------|---------------|
| Type coverage | 0-60 | All interface types present in the PRD (UI, TUI, Mobile, API, CLI) have corresponding TCs. If PRD has API endpoints, there are API TCs |
...
| Integration scenarios | 0-70 | TCs cover cross-feature or cross-interface scenarios (e.g., UI action triggers API call, CLI command affects UI state) where applicable and mentioned in the PRD |
