...
The mobile-test-cases.md must contain these sections:

- [ ] Frontmatter with `feature`, `sources`, `generated`
- [ ] Mobile Test Cases section with individual test cases
...
| Criterion | Points | What to check |
|-----------|--------|---------------|
| Steps are concrete actions | 0-90 | Each step describes a single, unambiguous user action. "Tap the Login button" not "Login". "Swipe left on the card" not "Navigate away" |
| Expected results are verifiable | 0-90 | Every expected result can be objectively verified: specific text, screen state, element visibility, data value. Not "should work" or "displays correctly" |
| Preconditions are explicit | 0-70 | TCs with dependencies (logged-in user, existing data, specific screen) declare them in Pre-conditions. No implicit assumptions |
...
| Integration scenarios | 0-70 | TCs cover cross-feature or cross-interface scenarios (e.g., mobile action triggers API call, notification flow) where applicable and mentioned in the PRD |
| Mobile coverage breadth | 0-60 | All mobile-related features described in the PRD have corresponding test cases. Every screen, flow, or view mentioned in the PRD is tested |
...
| Classification is correct | 0-30 | Each TC is classified as Mobile type. No UI, API, CLI, or TUI TCs in this file |
...
| Criterion | Points | What to check |
|-----------|--------|---------------|
| Pre-conditions are concrete and creatable | 0-30 | Score only pre-conditions that exist in the document — missing pre-conditions are penalized under D2 (Step Actionability). For pre-conditions that ARE listed, every one must describe HOW to create the required state (e.g., "a test user with profile photo uploaded"), not just assert it exists (e.g., "user profile exists"). If a pre-condition cannot be created using an isolated test fixture (temp directory, test container, mock server, etc.), the downstream script will generate a conditional skip without fixture. Deduct 10 pts per non-creatable pre-condition |
| Steps describe runtime behavior | 0-25 | No step describes reading source files (`.md`, `.swift`, `.kt`, `.json`), checking documentation content, or verifying file existence. Every step interacts with the running app (tap, swipe, screen navigation). Deduct 10 pts per static-file-check step |
...
| Every TC is implementable | 0-10 | No TC describes a scenario that requires unavailable infrastructure (e.g., "real device farm") without marking itself as non-implementable. Dead TCs generate unconditional skips — worse than no test at all. Deduct 5 pts per non-implementable TC without annotation |
...
- **Vague language without specificity**: -20 pts per instance ("tap button" without identifying which, "check screen" without expected state)
- **Cross-section inconsistency**: -30 pts per conflict (e.g., traceability table says TC-005 is UI but it's listed under Mobile)
