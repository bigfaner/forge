...
| Criterion | Points | What to check |
|-----------|--------|---------------|
| Steps are concrete actions | 0-90 | Each step describes a single, unambiguous CLI invocation. "`forge task query <task-id>`" not "List pending tasks". "`forge task add --title 'test'`" not "Create a task" |
