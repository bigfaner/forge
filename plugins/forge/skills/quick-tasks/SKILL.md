---
name: quick-tasks
description: Use for features (1-10 coding tasks, doc tasks unlimited) to generate tasks directly from proposal. No PRD or design needed.
---

# Quick Tasks

Generate executable tasks directly from a proposal document. For features (1-10 coding tasks, doc-type tasks unlimited) that don't need PRD or tech design.
...
<HARD-GATE>
Maximum 10 coding tasks (`coding.*` type). Doc-type tasks (`doc*` type prefix) are unlimited. If the proposal requires >10 coding tasks, STOP and recommend the full pipeline: `/write-prd` → `/tech-design` → `/breakdown-tasks`.
</HARD-GATE>
...
<HARD-RULE>
Enforce maximum 10 coding tasks (`coding.*` type). Doc-type tasks (`doc*` type prefix) are unlimited. If the In Scope section implies >10 coding tasks, STOP and recommend the full pipeline (`/write-prd` → `/tech-design` → `/breakdown-tasks`).
</HARD-RULE>
...
**Split by functional steps**: multiple independently verifiable steps in one bullet → separate tasks (coding tasks still ≤ 10, doc tasks unlimited).
...
- [ ] `docs/features/<slug>/tasks/` contains ≤10 coding task files + any number of doc task files
