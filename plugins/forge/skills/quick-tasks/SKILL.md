...
4. Classify task type by Affected Files (see Template Selection in Step 3)
5. For implementation tasks: set scope via Scope Assignment algorithm (same as breakdown-tasks — inspect file paths) and set `breaking: true` if task modifies shared interfaces, data models, or API contracts
