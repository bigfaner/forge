...
Every task receives a `type` field in its frontmatter. Assign based on the task's primary output:

| Type | When to assign |
|------|----------------|
| `implementation` | Default for source code, build config, or runtime config changes |
| `documentation` | Tasks producing only markdown, specs, or templates |
| `gate` | Quality-gate or stage-gate verification tasks |

Unrecognized tasks fall back to `implementation`. Test pipeline tasks are auto-generated by `forge task index`.
