...
`forge task index` auto-infers `type` from task ID patterns. No need to write `type` manually. Business tasks (IDs `1`–`4`) fall through to `"implementation"` fallback — expected behavior.
