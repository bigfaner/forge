...
When all tasks are `type: "doc"` (non-compilable, non-runnable output), skip **Step 0** (language) and **Step 5** (test tasks). **Step 6** (`forge task index`) is always mandatory — without `index.json`, `forge task claim` fails.
...
graph LR
    ...
    D -->|"skip 0, 5"| B["Steps 2–4"]
    D -->|No| A["Steps 0→2→3→4→5"]
    B --> S4["Step 4: Audit"]
    A --> S4
    S4 --> S6
    S6["Step 6: forge task index<br>(mandatory)"] --> S789["Steps 7–9"]
    style S6 fill:#fff3cd,stroke:#856404,stroke-width:2px
```
...
2. Assign new sequential IDs following the `<seq>` convention.
...
To add a fix task for a failing test: `forge task add --type <derived-fix-type> --title "Fix: <desc>" --source-task-id <TASK_ID> --block-source --var SOURCE_FILES="<paths>" --var TEST_SCRIPT="<test>" --var TEST_RESULTS="<results>" --description "<root cause>"`

**Fix-Type Derivation**: extract `TASK_CATEGORY` from claim output and map: `doc`/`eval` → `doc.fix`; `coding`/`test`/`validation`/`gate` → `coding.fix`.

## Step 6: Generate index.json via CLI

After all business task `.md` files (Step 3) and the task sizing audit (Step 4) are complete, run:
...
- [ ] `index.json` valid per schema, `forge task validate` passes
