...
| `coding.refactor` | Task restructures code without changing behavior |
| `doc` | Task produces only non-compilable, non-runnable output |
| `gate` | Quality-gate or stage-gate verification tasks |

Fallback: `coding.feature`. **Classify by output artifact, not intent.**

Quality-gate routing: Code types (`coding.*`) → run (compile + fmt + lint + test); `doc` → skip; `gate` → special.
