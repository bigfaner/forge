...
Rules for auto-extracting knowledge from the tech-design document after approval. This step captures knowledge that Step 7 (decision archiving) may have missed.
...
Mode context: `quick` when invoked via `/quick` pipeline, `full` when invoked via full pipeline. Parse the config output format `quick:<val> full:<val>` (e.g., `quick:true full:false`) and select the value matching the current mode.
