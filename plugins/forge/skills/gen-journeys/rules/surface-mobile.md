...
| React Native | `package.json` with `react-native` in dependencies | `react-native` + mobile-specific plugins | Not WebUI (react-native targets mobile, not browser) |
...
1. React Native vs WebUI: `react-native` in dependencies = Mobile. `react` without `react-native` = WebUI. Do not confuse the two.
...
3. Progressive Web App (PWA): If the project is primarily a website with PWA capabilities, classify as WebUI, not Mobile.
...
Mobile surface does not have mandatory derived Outcomes in the same way as CLI/API/WebUI due to its best-effort nature. Instead, use these common patterns as reference:
