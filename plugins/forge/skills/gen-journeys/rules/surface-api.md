...
**Test type**: API Functional Test. Verifies HTTP status codes, response schemas, authentication/authorization, and idempotency via HTTP client invocation. Generated test code MUST use `@api-functional` tags, NOT `@e2e`.
...
1. If `package.json` contains both `express`/`fastify` and `react`/`vue`, check for a frontend build output. If the server serves a SPA, classify as web (the API serves the frontend). If the server provides a pure API consumed by external clients, classify as api.
2. Server-side rendering (SSR) with template engines (EJS, Pug, Jinja2): if the primary output is HTML pages for browser consumption, classify as web. If the primary output is JSON/XML API responses, classify as api.
