...
> **Type sources**: `code-quality.simplify` — auto-generated by `forge task index` via PipelineRegistry (ID: `T-clean-code`), also created by `/clean-code` and `/simplify` skills. All `coding.*` subtypes are business task types assigned by `/breakdown-tasks` or `/quick-tasks`.
