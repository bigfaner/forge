# Surface: mobile — Run-Tests Orchestration

本规则文件定义 run-tests skill 对 mobile surface 的编排序列。消费方为 SKILL.md 调度器。
...
| test | `just mobile-test` | 测试通过 | 测试失败 | 测试环境异常（需重试） | 进入 teardown |
