...
**Skip:**
- No test scripts exist under `tests/` (run `/gen-test-scripts` first)
...
| `tests/` directory with test scripts (at least one journey) | Run `/gen-test-scripts` first |
...
<Test-Dir-Note>
Test script location depends on surface configuration:
- **Single surface**: `tests/<journey>/` (no surface-key directory layer)
- **Multi surface**: `tests/<surfaceKey>/<journey>/` (key-based directory layer)

The journey discovery (Step 1.5) reads from `docs/features/<slug>/testing/` and passes journey names to `just <recipe-prefix>test <journey>`. The justfile recipe (generated by init-justfile) handles the actual path resolution to `tests/` subdirectories.
</Test-Dir-Note>
