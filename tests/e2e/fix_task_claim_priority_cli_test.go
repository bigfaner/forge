...
// fixClaimTaskEntry represents a single task in the index.json fixture.
type fixClaimTaskEntry struct {
	ID            string   `json:"id"`
...
// fixClaimIndexFixture represents the top-level index.json structure.
type fixClaimIndexFixture struct {
	Feature      string                      `json:"feature"`
	StatusEnum   []string                    `json:"statusEnum"`
	PriorityEnum []string                    `json:"priorityEnum"`
	Tasks        map[string]fixClaimTaskEntry `json:"tasks"`
}
...
func setupFeatureFixture(t *testing.T, tasks map[string]fixClaimTaskEntry) string {
	t.Helper()
	dir := t.TempDir()
...
func TestTC_001_PendingFixTaskBlocksDependentBusinessTask(t *testing.T) {
	...
	projectRoot := setupFeatureFixture(t, map[string]fixClaimTaskEntry{
		"3": {
