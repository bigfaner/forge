...
// ==============================================================================
// forge feature set — CLI e2e tests for feature: feature-set-command
...
func forgeFeatureSet(t *testing.T, projectRoot, slug string) (string, int) {
	t.Helper()
	cmd := exec.Command("forge", "feature", "set", slug)
...
func forgeFeature(t *testing.T, projectRoot string) (string, int) {
	t.Helper()
	cmd := exec.Command("forge", "feature")
	cmd.Env = append(os.Environ(), "CLAUDE_PROJECT_DIR="+projectRoot)
	out, err := cmd.CombinedOutput()
	exitCode := 0
	if err != nil {
...
func forgeFeatureVerbose(t *testing.T, projectRoot string) (string, int) {
	t.Helper()
	cmd := exec.Command("forge", "feature", "-v")
	cmd.Env = append(os.Environ(), "CLAUDE_PROJECT_DIR="+projectRoot)
	out, err := cmd.CombinedOutput()
	exitCode := 0
	if err != nil {
...
func forgeFeaturePositional(t *testing.T, projectRoot, slug string) (string, int) {
	t.Helper()
	cmd := exec.Command("forge", "feature", slug)
	cmd.Env = append(os.Environ(), "CLAUDE_PROJECT_DIR="+projectRoot)
	out, err := cmd.CombinedOutput()
	exitCode := 0
	if err != nil {
...
func TestTC_019_VerboseShowsNoneWhenNoFeatureSet(t *testing.T) {
	projectRoot := setupProjectDir(t)
...
func TestTC_020_VerboseFlagLocalToFeatureCommandOnly(t *testing.T) {
	projectRoot := setupProjectDir(t)
	...
	cmd := exec.Command("forge", "feature", "set", "-v", "my-feature")
	cmd.Env = append(os.Environ(), "CLAUDE_PROJECT_DIR="+projectRoot)
	...
	cmd3 := exec.Command("forge", "feature", "list", "-v")
