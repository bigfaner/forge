...
describe('Skill/Agent file content checks', () => {
  ...
  test('TC-007: run-tasks Breaking Gate uses just test', () => {
    ...
    assert.ok(breakingGateIdx !== -1, 'Expected "Breaking Task Gate" section to exist in run-tasks.md');
  ...
  test('TC-014: gen-test-scripts Step 4 uses just e2e-verify', () => {
    ...
    assert.ok(
      fileNotContains(content, "grep -r '// VERIFY:'"),
      'Expected raw "grep -r \'// VERIFY:\'" NOT to appear in gen-test-scripts/SKILL.md',
    );
  });
  ...
  test('TC-018: init-justfile generates e2e-setup target', () => {
    ...
    assert.ok(
      fileContains(content, 'playwright install chromium'),
      'Expected "playwright install chromium" in e2e-setup recipe',
    );
  });
...
describe('just command execution', () => {
  const tmpE2eDir = join(PROJECT_ROOT, 'tests', 'e2e');
  const tmpFeatureDir = join(tmpE2eDir, 'test-feature');
  const tmpMyFeatureDir = join(tmpE2eDir, 'my-feature');
  ...
  after(() => {
    ...
    // Remove tests/e2e only if it was created by us (no package.json = we created it)
    if (existsSync(tmpE2eDir) && !existsSync(join(tmpE2eDir, 'package.json'))) {
      rmSync(tmpE2eDir, { recursive: true, force: true });
    }
  });
  ...
  test('TC-003: just e2e-verify exits 1 when VERIFY markers present', () => {
    ...
    assert.equal(result.exitCode, 1, 'Expected exit code 1 when VERIFY markers present');
  ...
  test('TC-004: just e2e-verify exits 0 when no VERIFY markers', () => {
    ...
    const result = runCli('just e2e-verify --feature test-feature');
    assert.equal(result.exitCode, 0, 'Expected exit code 0 when no VERIFY markers');
    assert.ok(
  ...
  test('TC-009: just e2e-setup exits 1 when package.json missing', () => {
    ...
    assert.equal(result.exitCode, 1, 'Expected exit code 1 when tests/e2e/package.json missing');
    const output = result.stdout + result.stderr;
    assert.ok(
  ...
  test('TC-010: just e2e-setup exits 0 with OK message when deps ready', () => {
    ...
    const result = runCli('just e2e-setup');
    assert.equal(result.exitCode, 0, 'Expected exit code 0 when deps are ready');
    assert.ok(
  ...
  test('TC-011: just e2e-verify exits 1 when feature flag missing', () => {
    ...
    assert.equal(result.exitCode, 1, 'Expected exit code 1 when --feature argument is missing');
    const output = result.stdout + result.stderr;
    assert.ok(
  ...
  test('TC-020: just e2e-setup is idempotent', () => {
    ...
    assert.equal(result1.exitCode, 0, 'Expected first run to exit 0');
    assert.equal(result2.exitCode, 0, 'Expected second run to exit 0');
    assert.ok(
