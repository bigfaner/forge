...
// ==============================================================================
// CLI list reverse chronological tests — Journey: feature-management
// Tests verify that forge proposal and forge feature list sort output
...
// createFeatureWithManifest creates a feature directory with manifest.md.
func createFeatureWithManifest(t *testing.T, projectDir, slug string, mtime time.Time, withManifest bool) {
	t.Helper()
...
// TC-019: forge proposal lists proposals sorted by created date descending
// Traceability: TC-001 -> Proposal Success Criterion "forge proposal lists proposals newest-first by created date" + Task 1 AC-1
func TestTC_019_ProposalListSortedByCreatedDescending(t *testing.T) {
	...
	output := runForge(t, projectDir, "proposal")
...
// TC-020: forge proposal handles proposals without created frontmatter (mtime fallback)
...
func TestTC_020_ProposalListMtimeFallback(t *testing.T) {
	// Step 1: Create a temporary project with go.mod
	projectDir := createTempProject(t)
	...
	output, exitCode := runForgeRaw(t, projectDir, "proposal")
...
// TC-021: forge proposal with empty proposals directory
// Traceability: TC-003 -> Task 1 AC-3 (existing tests pass, empty case)
func TestTC_021_ProposalListEmptyDirectory(t *testing.T) {
...
// TC-022: forge feature list sorts features by manifest mtime descending
...
func TestTC_022_FeatureListSortedByMtimeDescending(t *testing.T) {
	// Step 1: Create a temporary project with go.mod
	projectDir := createTempProject(t)

	// Step 2: Create features with manifest.mtimes: 2026-01-01, 2026-03-15, 2026-05-16
	createFeatureWithManifest(t, projectDir, "feature-alpha", time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC), true)
	createFeatureWithManifest(t, projectDir, "feature-beta", time.Date(2026, 3, 15, 0, 0, 0, 0, time.UTC), true)
	createFeatureWithManifest(t, projectDir, "feature-gamma", time.Date(2026, 5, 16, 0, 0, 0, 0, time.UTC), true)

	// Step 3: Run forge feature list
	output := runForge(t, projectDir, "feature", "list")
...
// TC-023: forge feature list sorts features with missing manifest to the end
// Traceability: TC-005 -> Task 2 AC-2 "Features with missing/unreadable manifest sort to the end"
func TestTC_023_FeatureListMissingManifestToEnd(t *testing.T) {
	// Step 1: Create a temporary project with go.mod
	projectDir := createTempProject(t)

	// Step 2: Create one feature with a valid manifest.md (recent mtime)
	createFeatureWithManifest(t, projectDir, "feature-with-manifest", time.Date(2026, 5, 16, 0, 0, 0, 0, time.UTC), true)

	// Step 3: Create one feature directory without manifest.md
	createFeatureWithManifest(t, projectDir, "feature-no-manifest", time.Time{}, false)
