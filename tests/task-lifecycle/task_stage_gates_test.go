...
// sgTestDir creates a temporary feature directory with the given task files.
...
func sgTestDir(t *testing.T, featureSlug string, taskFiles []string) (string, func()) {
	t.Helper()
	tmpRoot := t.TempDir()
	...
	for _, tf := range taskFiles {
		...
		filename := parts[0]
		content := sgDefaultTaskContent(filename)
...
// sgDefaultTaskContent returns a minimal valid task markdown file.
func sgDefaultTaskContent(filename string) string {
...
// sgTaskContentWithType returns task content with a specific type field.
func sgTaskContentWithType(filename, taskType string) string {
	id := strings.TrimSuffix(filename, ".md")
	if idx := strings.Index(id, "-"); idx > 0 {
		id = id[:idx]
	}
...
// sgParseFrontmatter parses YAML frontmatter from a markdown file.
func sgParseFrontmatter(t *testing.T, path string) map[string]interface{} {
	t.Helper()
...
// sgParseIndexJSON reads and parses index.json from a feature directory.
func sgParseIndexJSON(t *testing.T, tmpRoot, featureSlug string) map[string]interface{} {
	t.Helper()
...
// sgRunForgeIndex runs forge task index in the given project root using the binary
// built by testkit init. Returns stdout, stderr, exit code.
func sgRunForgeIndex(t *testing.T, tmpRoot, featureSlug string, extraArgs ...string) (string, string, int) {
	t.Helper()
...
func TestTSG_001_GeneratesSummaryAndGateForQualifyingPhases(t *testing.T) {
	...
	tmpRoot, cleanup := sgTestDir(t, featureSlug, []string{
		"1.1-task-a.md",
		"1.2-task-b.md",
		"2.1-task-c.md",
		"2.2-task-d.md",
	...
	_, _, exitCode := sgRunForgeIndex(t, tmpRoot, featureSlug)
	...
	fm1 := sgParseFrontmatter(t, filepath.Join(tasksDir, "1.summary.md"))
	deps1, ok := fm1["dependencies"].([]interface{})
	require.True(t, ok)
	depStrs1 := sgToStringSlice(deps1)
	assert.Contains(t, depStrs1, "1.1")
	assert.Contains(t, depStrs1, "1.2")

	gateFm1 := sgParseFrontmatter(t, filepath.Join(tasksDir, "1.gate.md"))
	gateDeps1, ok := gateFm1["dependencies"].([]interface{})
	require.True(t, ok)
	assert.Contains(t, sgToStringSlice(gateDeps1), "1.summary")
	assert.Equal(t, true, gateFm1["breaking"])

	// Verify dependency wiring for phase 2
	fm2 := sgParseFrontmatter(t, filepath.Join(tasksDir, "2.summary.md"))
	deps2, ok := fm2["dependencies"].([]interface{})
	require.True(t, ok)
	depStrs2 := sgToStringSlice(deps2)
	assert.Contains(t, depStrs2, "2.1")
	assert.Contains(t, depStrs2, "2.2")

	gateFm2 := sgParseFrontmatter(t, filepath.Join(tasksDir, "2.gate.md"))
	gateDeps2, ok := gateFm2["dependencies"].([]interface{})
	require.True(t, ok)
	assert.Contains(t, sgToStringSlice(gateDeps2), "2.summary")
...
func TestTSG_002_CorrectDependencyWiringForGateTasks(t *testing.T) {
	...
	fm := sgParseFrontmatter(t, filepath.Join(tasksDir, "1.summary.md"))
	deps, ok := fm["dependencies"].([]interface{})
	require.True(t, ok)
	depStrs := sgToStringSlice(deps)
	sort.Strings(depStrs)
	assert.Equal(t, []string{"1.1", "1.2", "1.3"}, depStrs)

	// Verify gate depends only on summary
	gateFm := sgParseFrontmatter(t, filepath.Join(tasksDir, "1.gate.md"))
	gateDeps, ok := gateFm["dependencies"].([]interface{})
	require.True(t, ok)
	assert.Equal(t, []string{"1.summary"}, sgToStringSlice(gateDeps))
...
func TestTSG_004_ExcludesTestOnlyPhases(t *testing.T) {
	...
	tmpRoot, cleanup := sgTestDir(t, featureSlug, []string{
		"1.1.md:" + sgTaskContentWithType("1.1.md", "feature"),
		"T-test-gen-scripts-cli.md:" + sgTaskContentWithType("T-test-gen-scripts-cli.md", "testTask"),
		"T-test-run-cli.md:" + sgTaskContentWithType("T-test-run-cli.md", "testTask"),
	})
	defer cleanup()
...
func TestTSG_006_IdempotentRerunPreservesExistingFiles(t *testing.T) {
	...
	summaryModTime := sgGetFileModTime(t, summaryPath)
	gateModTime := sgGetFileModTime(t, gatePath)
	...
	assert.Equal(t, summaryModTime, sgGetFileModTime(t, summaryPath), "summary mod time should be unchanged")
	assert.Equal(t, gateModTime, sgGetFileModTime(t, gatePath), "gate mod time should be unchanged")
}
...
`
	...
	gateDeps, ok := gateFm["dependencies"].([]interface{})
	require.True(t, ok)
	assert.Contains(t, sgToStringSlice(gateDeps), "1.summary")
}
...
func TestTSG_011_GeneratedTasksInIndexJsonWithCorrectType(t *testing.T) {
	...
	idx := sgParseIndexJSON(t, tmpRoot, featureSlug)
...
func TestTSG_012_PrintsSummaryLinePerQualifyingPhase(t *testing.T) {
	...
	stdout, stderr, exitCode := sgRunForgeIndex(t, tmpRoot, featureSlug)
	assert.Equal(t, 0, exitCode)
...
func TestTSG_017_NoTestFlagRemoved(t *testing.T) {
	...
	_, stderr, exitCode := sgRunForgeIndex(t, tmpRoot, featureSlug, "--no-test")
...
`
	...
	normalTasks := []string{
		"1.1.md:" + sgDefaultTaskContent("1.1.md"),
		"1.2.md:" + sgDefaultTaskContent("1.2.md"),
	}
...
// sgToStringSlice converts []interface{} to []string.
func sgToStringSlice(in []interface{}) []string {
...
// sgGetFileModTime returns the modification time of a file.
func sgGetFileModTime(t *testing.T, path string) time.Time {
	t.Helper()
