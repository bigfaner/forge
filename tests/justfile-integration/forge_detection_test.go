...
// Custom recipes preserved outside boundary markers.
// The custom recipe name has evolved (claude-p replaces claude/claude-c).
func TestTC_FJ_010_CustomRecipesPreservedOutsideBoundaryMarkers(t *testing.T) {
	content := getJustfile(t)
	// Check that custom recipes exist outside boundary markers
	startMarker := "# --- forge standard recipes ---"
	endMarker := "# --- end forge standard recipes ---"
	startIdx := strings.Index(content, startMarker)
	endIdx := strings.Index(content, endMarker)
	assert.NotEqual(t, -1, startIdx, "Expected start boundary marker")
	assert.NotEqual(t, -1, endIdx, "Expected end boundary marker")

	beforeMarkers := content[:startIdx]
	afterMarkers := content[endIdx+len(endMarker):]
	// Verify at least one custom recipe exists outside boundary markers
	hasCustomRecipe :=
		strings.Contains(beforeMarkers, "claude-p:") ||
			strings.Contains(afterMarkers, "claude-p:") ||
			strings.Contains(beforeMarkers, "claude:") ||
			strings.Contains(afterMarkers, "claude:") ||
			strings.Contains(beforeMarkers, "install-forge:") ||
			strings.Contains(afterMarkers, "install-forge:")
	assert.True(t, hasCustomRecipe, "Expected custom recipes outside boundary markers")
}
...
// getProjectDetectionContent reads the project detection rules content.
func getProjectDetectionContent(t *testing.T) string {
	t.Helper()
	return testkit.ReadProjectFile(t, "../plugins/forge/skills/init-justfile/rules/project-detection.md")
}
...
// Detection logic checks for package.json as frontend signal (now in rules/project-detection.md).
func TestTC_DET_001_DetectionChecksPackageJsonAsFrontendSignal(t *testing.T) {
	content := getProjectDetectionContent(t)
	assert.True(t, strings.Contains(content, "package.json"),
		"Expected detection logic to check for package.json")
	// Verify it is mapped as a frontend signal
	assert.True(t, strings.Contains(content, "frontend"),
		"Expected package.json mapped to frontend signal")
}

// Traceability: TC-DET-002 -> AC: Detection checks go.mod (backend signal)
// Detection logic checks for go.mod as backend signal (now in rules/project-detection.md).
...
// Detection logic checks for Cargo.toml as backend signal (now in rules/project-detection.md).
...
// Detection logic checks for pyproject.toml as backend signal (now in rules/project-detection.md).
...
// Classification produces error when no known markers detected (now in rules/project-detection.md).
...
// Detection mapping is now in rules/project-detection.md (SKILL.md delegates via Step 1a).
func TestTC_DET_019_DetectionCorrectlyMapsSignalsToFrontendBackendCategories(t *testing.T) {
	content := getProjectDetectionContent(t)

	// frontend signal should be mapped
	assert.True(t, strings.Contains(content, "frontend"),
		"Expected \"frontend\" classification in detection rules")

	// backend signal should be mapped
	assert.True(t, strings.Contains(content, "backend"),
		"Expected \"backend\" classification in detection rules")
