...
// The mixed template targets mixed projects with per-surface recipes
// (frontend-compile, backend-compile, etc.) instead of case/esac dispatch.
func TestTC_006_MixedProjectDetectionGeneratesScopeAwareJustfile(t *testing.T) {
	...
	// Mixed template should have frontend_dir and backend_dir variables
	assert.True(t, fileContains(mixedTemplate, `frontend_dir`),
		`Expected frontend_dir variable in mixed template`)
	assert.True(t, fileContains(mixedTemplate, `backend_dir`),
		`Expected backend_dir variable in mixed template`)
	// Mixed template should have per-surface recipes (frontend-*, backend-*)
	assert.True(t, fileContains(mixedTemplate, `frontend-compile`),
		`Expected frontend-compile recipe in mixed template`)
	assert.True(t, fileContains(mixedTemplate, `backend-compile`),
		`Expected backend-compile recipe in mixed template`)
}
...
// The error case is now defined in rules/project-detection.md (delegated from SKILL.md Step 1a).
func TestTC_018_NoMarkerFilesDetectedCausesInitJustfileToError(t *testing.T) {
	// Check project-detection.md for the error case (SKILL.md delegates to it)
	content := testkit.ReadProjectFile(t, "../plugins/forge/skills/init-justfile/rules/project-detection.md")
	...
	assert.True(t, hasError,
		"Expected error handling description for no project markers in project-detection.md")
}
