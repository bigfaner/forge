...
func TestTC_001_ListTypesIncludesDriftTypeWithCorrectDescription(t *testing.T) {
	bin := buildForgeFromSource(t)
	cmd := exec.Command(bin, "task", "list-types")
...
func TestTC_007_TTest5DescriptionIncludesDriftDetection(t *testing.T) {
	// Verify breakdown-tasks SKILL.md references drift detection for T-test-5
	...
	assert.Contains(t, content, "T-test-5",
		"breakdown-tasks SKILL.md should mention T-test-5")
	assert.True(t,
		strings.Contains(strings.ToLower(content), "drift") ||
			strings.Contains(content, "consolidate"),
		"breakdown-tasks SKILL.md T-test-5 description should reference consolidation/drift detection")
}
...
// Traceability: TC-013 -> Task 3 AC (breakdown-tasks SKILL.md references T-test-5 drift detection)
...
// buildForgeFromSource builds forge from source and returns the binary path.
func buildForgeFromSource(t *testing.T) string {
	t.Helper()
	var buildErr error
	forgeBinOnce.Do(func() {
		binDir, err := os.MkdirTemp("", "forge-drift-e2e-bin-*")
		if err != nil {
			buildErr = fmt.Errorf("create bin temp dir: %w", err)
			return
		}
		// Use .exe extension for Windows compatibility with exec.Command
		exeName := "forge"
...
// parseIndexTasks reads index.json and returns the tasks map.
func parseIndexTasks(t *testing.T, indexPath string) map[string]interface{} {
	t.Helper()
