...
func TestReadAutoConfig_MissingConfig(t *testing.T) {
	...
	// Missing config should return defaults: e2eTest true, consolidateSpecs true, cleanCode false
	if !auto.E2eTest.Quick || !auto.E2eTest.Full {
		t.Errorf("E2eTest defaults = %+v, want {Quick:true Full:true}", auto.E2eTest)
	}
...
`
	...
	if !auto.E2eTest.Quick || !auto.E2eTest.Full {
		t.Errorf("E2eTest defaults = %+v, want true/true", auto.E2eTest)
	}
	if !auto.ConsolidateSpecs.Quick || !auto.ConsolidateSpecs.Full {
		t.Errorf("ConsolidateSpecs defaults = %+v, want true/true", auto.ConsolidateSpecs)
	}
	if auto.CleanCode.Quick || auto.CleanCode.Full {
		t.Errorf("CleanCode defaults = %+v, want false/false", auto.CleanCode)
	}
}
