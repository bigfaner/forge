...
// writeMultiSurfaceForgeConfig creates a .forge/config.yaml with multiple surfaces.
// surfaces is a map of surface-key -> surface-type.
func writeMultiSurfaceForgeConfig(t *testing.T, projectRoot string, surfaces map[string]string) {
	t.Helper()
	forgeDir := filepath.Join(projectRoot, ".forge")
	if err := os.MkdirAll(forgeDir, 0755); err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(filepath.Join(forgeDir, "config.yaml"), []byte(buf.String()), 0644); err != nil {
		t.Fatal(err)
	}
}
...
func TestBuildIndex_WithTestTasks(t *testing.T) {
	...
	// 2 business tasks + 2 stage-gates + test pipeline tasks >= 4
	total := result.NewCount + result.UpdatedCount
	if total < 4 {
		t.Errorf("total tasks = %d (new=%d, updated=%d), want at least 4", total, result.NewCount, result.UpdatedCount)
	}
...
func TestBuildIndex_MultiProfile(t *testing.T) {
	...
	// 1 business task + test pipeline tasks (surfaces configured)
	total := result.NewCount + result.UpdatedCount
	if total < 2 {
		t.Errorf("total = %d (new=%d, updated=%d), want at least 2", total, result.NewCount, result.UpdatedCount)
	}
}
...
func TestBuildIndex_StageGatesWithNoProfiles(t *testing.T) {
	...
	// No E2E test tasks generated (no profiles), but non-E2E auto tasks may exist
	...
	for _, e := range entries {
		...
		// Non-E2E auto-generated tasks (e.g., quick-drift-detection) are expected with default auto config
		if strings.HasPrefix(name, "quick-") || strings.HasPrefix(name, "gen-journeys-") {
			continue
		}
...
func TestBuildIndex_FixTaskPassesValidation(t *testing.T) {
	...
	// Use empty mode to avoid test pipeline (which requires surfaces config)
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "")
...
func TestBuildIndex_MissingTypeAllowedForAutoGenTasks(t *testing.T) {
	// Use empty mode (no proposal/PRD) to avoid test pipeline triggering,
	// which would require surfaces configuration.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "")
...
func TestBuildIndex_ValidationTasksGenerated(t *testing.T) {
	...
	// BuildIndex produces: 1 business + 2 gates + test pipeline tasks
	// (base structure + per-type test tasks generated from surfaces)
	total := result.NewCount + result.UpdatedCount
	if total < 3 {
		t.Errorf("total tasks = %d (new=%d, updated=%d), want at least 3", total, result.NewCount, result.UpdatedCount)
	}
...
// --- Task 6: BuildIndex fix-task migration tests ---

// writeIndexWithFixTasks creates an index.json with fix-tasks that have SourceTaskID "T-test-run".
// This simulates a pre-migration state where T-test-run was a single task (before surface-key splitting).
func writeIndexWithFixTasks(t *testing.T, indexPath string, fixTasks []Task, runTestTask *Task) {
	t.Helper()
	index := NewTaskIndex("test-feature")

	// Add the old T-test-run task if provided
	if runTestTask != nil {
		index.SetTask("run-test", *runTestTask)
	}

	// Add fix-tasks
	for _, ft := range fixTasks {
		key := ft.ID
		index.SetTask(key, ft)
	}
...
func TestBuildIndex_Migration_MultiSurfaceRemapsFixTasks(t *testing.T) {
	// AC1: Multi-surface project index.json with fix-tasks having SourceTaskID "T-test-run"
	// should auto-remap to "T-test-run-{first-surface-key}" during BuildIndex.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "quick")
	writeMultiSurfaceForgeConfig(t, projectRoot, map[string]string{
		"backend":  "api",
		"frontend": "web",
	})

	// Create a business task so the feature is not empty
	writeTaskMDWithType(t, tasksDir, "1-feat.md", "1", "Feature Task", TypeCodingFeature, nil)

	// Write an index.json with fix-tasks pointing to the old T-test-run
	writeIndexWithFixTasks(t, indexPath, []Task{
		{
			ID: "fix-1", Title: "Fix test failure", Type: TypeCodingFix,
			SourceTaskID: "T-test-run", Status: "in_progress",
			File: "fix-1.md", Record: "records/fix-1.md",
		},
	}, &Task{
		ID: "T-test-run", Title: "Run e2e Tests", Type: TypeTestRun,
		Status: "completed", File: "run-test.md", Record: "records/run-test.md",
	})
	...
	// fix-1 should now point to T-test-run-backend (first in execution order)
	fixTask, ok := idx.Tasks["fix-1"]
	if !ok {
		t.Fatal("fix-1 not in index")
	}
	if fixTask.SourceTaskID != "T-test-run-backend" {
		t.Errorf("fix-1 SourceTaskID = %q, want %q", fixTask.SourceTaskID, "T-test-run-backend")
	}

	// Old T-test-run should no longer exist as a standalone key
	// (it should be replaced by T-test-run-backend)
	if _, hasOldKey := idx.Tasks["run-test"]; hasOldKey {
		// Check if the old key's ID is still T-test-run (not remapped)
		if oldTask, ok := idx.Tasks["run-test"]; ok && oldTask.ID == "T-test-run" && oldTask.Status == "completed" {
			t.Error("old 'run-test' key with ID 'T-test-run' should have been removed/replaced")
		}
	}

	// T-test-run-backend should be in the index (generated by test pipeline)
	_, hasBackend := idx.Tasks["run-test-backend"]
	if !hasBackend {
		var keys []string
		for k := range idx.Tasks {
			keys = append(keys, k)
		}
		t.Errorf("run-test-backend not in index. Keys: %v", keys)
	}

	_ = result
}

func TestBuildIndex_Migration_SingleSurfaceNoMigration(t *testing.T) {
	// AC2: Single surface project (surfaces: api) has no migration behavior.
	// T-test-run stays unchanged.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "quick")
	writeForgeConfig(t, projectRoot) // single surface: api

	writeTaskMDWithType(t, tasksDir, "1-feat.md", "1", "Feature Task", TypeCodingFeature, nil)

	// Write index with fix-tasks pointing to T-test-run
	writeIndexWithFixTasks(t, indexPath, []Task{
		{
			ID: "fix-1", Title: "Fix test failure", Type: TypeCodingFix,
			SourceTaskID: "T-test-run", Status: "in_progress",
	...
	// fix-1 should STILL point to T-test-run (no remapping for single surface)
	fixTask, ok := idx.Tasks["fix-1"]
	if !ok {
		t.Fatal("fix-1 not in index")
	}
	if fixTask.SourceTaskID != "T-test-run" {
		t.Errorf("fix-1 SourceTaskID = %q, want %q (single surface: no migration)", fixTask.SourceTaskID, "T-test-run")
	}
}

func TestBuildIndex_Migration_OldRunTestRemoved(t *testing.T) {
	// AC3: After migration, old T-test-run entry is removed from index.json.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "quick")
	writeMultiSurfaceForgeConfig(t, projectRoot, map[string]string{
		"backend":  "api",
		"frontend": "web",
	...
	// Write index with old T-test-run and fix-tasks
	writeIndexWithFixTasks(t, indexPath, []Task{
		{
			ID: "fix-1", Title: "Fix test failure", Type: TypeCodingFix,
			SourceTaskID: "T-test-run", Status: "in_progress",
	...
	// The old "run-test" key should not exist with ID "T-test-run"
	// (it should be removed during migration, and replaced by run-test-backend/run-test-frontend)
	for key, task := range idx.Tasks {
		if task.ID == "T-test-run" && key == "run-test" {
			t.Errorf("old entry key=%q id=%q should have been removed during migration", key, task.ID)
		}
	}
}

func TestBuildIndex_Migration_StatusPreserved(t *testing.T) {
	// Multi-surface project: old T-test-run status/blocked-reason is copied
	// to T-test-run-{first-surface-key}.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "quick")
	writeMultiSurfaceForgeConfig(t, projectRoot, map[string]string{
		"backend":  "api",
		"frontend": "web",
	...
	// Old T-test-run has a specific status and blocked-reason
	writeIndexWithFixTasks(t, indexPath, []Task{
		{
			ID: "fix-1", Title: "Fix test failure", Type: TypeCodingFix,
			SourceTaskID: "T-test-run", Status: "in_progress",
		...
		ID: "T-test-run", Title: "Run e2e Tests", Type: TypeTestRun,
		Status: "blocked", BlockedReason: "upstream failure",
		File: "run-test.md", Record: "records/run-test.md",
	})
	...
	// T-test-run-backend should inherit the old T-test-run status and blocked-reason
	backendTask, ok := idx.Tasks["run-test-backend"]
	if !ok {
		t.Fatal("run-test-backend not in index")
	}
	if backendTask.Status != "blocked" {
		t.Errorf("run-test-backend Status = %q, want %q", backendTask.Status, "blocked")
	}
	if backendTask.BlockedReason != "upstream failure" {
		t.Errorf("run-test-backend BlockedReason = %q, want %q", backendTask.BlockedReason, "upstream failure")
	}
}

func TestBuildIndex_Migration_NoFixTasks_NoOp(t *testing.T) {
	// When there are no fix-tasks with SourceTaskID "T-test-run", migration is a no-op.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "quick")
	writeMultiSurfaceForgeConfig(t, projectRoot, map[string]string{
		"backend":  "api",
		"frontend": "web",
	...
	// Index with no fix-tasks
	writeIndexWithFixTasks(t, indexPath, nil, nil)
	...
	// No fix-tasks should exist
	for key, task := range idx.Tasks {
		if task.SourceTaskID != "" {
			t.Errorf("unexpected SourceTaskID on task %s: %q", key, task.SourceTaskID)
		}
	}
}

func TestBuildIndex_Migration_FixTasksPointToOtherSource_NoOp(t *testing.T) {
	// Fix-tasks with SourceTaskID other than "T-test-run" should NOT be migrated.
	projectRoot, tasksDir, indexPath := setupBuildEnv(t, "quick")
	writeMultiSurfaceForgeConfig(t, projectRoot, map[string]string{
		"backend":  "api",
		"frontend": "web",
	...
	// Fix-task pointing to a business task (not T-test-run)
	writeIndexWithFixTasks(t, indexPath, []Task{
		{
			ID: "fix-1", Title: "Fix code issue", Type: TypeCodingFix,
			SourceTaskID: "1", Status: "in_progress",
			File: "fix-1.md", Record: "records/fix-1.md",
		},
	}, nil)
	...
	// fix-1 should still point to "1" (not remapped)
	fixTask, ok := idx.Tasks["fix-1"]
	if !ok {
		t.Fatal("fix-1 not in index")
	}
