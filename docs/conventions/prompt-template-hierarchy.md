...
- **`<TASK-CONSTRAINTS>`** — 模板级工作流约束。定义模板自身对 agent 执行流程的强制要求（如"必须通过 Skill 调用，禁止直接执行"）。由模板设计者设定。已用于 `eval-journey`、`eval-contract`、`code-quality-simplify`、`test-gen-scripts`、`test-run`、`test-gen-contracts`、`test-gen-journeys` 等 prompt templates。
