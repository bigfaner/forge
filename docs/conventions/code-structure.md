...
|:----:|--------|----------|----------|----------|
| CS-1 | `cmd.Debugf` 重复定义 | TECH-code-structure-002 | `cmd/output.go` 和 `base/output.go` 双重定义 | 仅保留 `base.Debugf` |
...
| CS-5 | `Scope` deprecated 字段 | TECH-code-structure-005 | 保留用于迁移检测，已正确标注 | 迁移完成后移除 |
