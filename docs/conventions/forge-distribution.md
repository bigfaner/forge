...
│       ├── intent-propagation.md
│       ├── knowledge-extraction.md
│       ├── profile-detection.md
│       ├── sitemap.json
│       ├── step0-profile-resolution.md
│       └── type-assignment.md
...
│   ├── tasks/                # 任务文件
│   └── testing/              # 测试脚本（运行时生成）
├── tests/e2e/                # E2E 回归测试（通过 `forge test promote` 晋升标签管理）
└── .forge/
...
- `tests/e2e/` 测试通过标签管理（`forge test promote`），不使用文件迁移
