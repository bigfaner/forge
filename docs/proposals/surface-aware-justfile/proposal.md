...
**timeout 覆盖范围**：`test.timeout`（默认 300 秒）约束 run-tests 从 `just dev` 开始到 `just test-teardown` 完成的整个编排序列的总耗时上限。各阶段共享此配额——如果 probe 阶段消耗了 60 秒（30 次 x 2 秒），test 阶段实际可用时间为 240 秒。run-tests 在每个编排步骤前检查剩余时间，若剩余时间不足以完成下一个步骤的最低估计耗时，则跳过后续步骤并执行 teardown。各步骤的最低估计耗时定义如下：
- **dev 步骤**：最低估计 = 5 秒（仅启动进程并写 PID 文件）
- **probe 步骤**：最低估计 = 1 次 probe 间隔（默认 2 秒）
- **test 步骤**：最低估计 = 从 `test.timeout` 总配额中减去已消耗时间再乘以 0.3 的保底比例（确保 test 步骤至少获得总配额 30% 的执行机会），且最低不低于 30 秒。例如：总配额 300 秒，probe 消耗 60 秒，剩余 240 秒，test 最低估计 = max(240 × 0.3, 30) = 72 秒。当剩余时间 > 最低估计时，test 步骤正常执行（其实际运行时间不受最低估计限制，仍受总配额约束）
- **teardown 步骤**：最低估计 = 10 秒（SIGTERM + 等待 + SIGKILL fallback）

probe 配方体内的重试循环（max_retries x interval）独立于 `test.timeout`——probe 配方体有自己的超时上限（默认 30 x 2 = 60 秒），但 run-tests 的全局超时可能在 probe 未完成时触发中止
...
- **编排序列**（`## 编排序列` 段，供 run-tests 消费）：该 surface 类型的测试执行流程（如 web = dev → probe → test → teardown），定义步骤顺序、退出码门控逻辑和**退出码处理表**。退出码处理表是规则文件的强制性结构元素，定义了每个编排步骤可能返回的退出码及其对应的后续动作。新增退出码时必须同步更新此表，否则 run-tests 会按默认行为（通用失败 → teardown）处理未定义的退出码。示例如下：

  | 步骤 | 退出码 | 语义 | run-tests 后续动作 |
  |------|--------|------|-------------------|
  | probe | 0 | 服务健康 | 继续下一步（test） |
  | probe | 1 | 通用失败（连接被拒绝等） | 按默认重试策略处理 |
  | probe | 2 | 端口冲突（EADDRINUSE） | 立即中止所有重试，执行 teardown |
  | probe | 3 | 连接超时（端口有进程但不响应） | 跳过后续 probe 重试，执行 teardown |
...
**原子性保证**：阶段 1-4 必须在**同一 PR** 中完成，允许 PR 内按阶段拆分为多个逻辑提交（便于 code review 分阶段审查）。PR 合并前必须通过集成测试验证所有阶段联合行为一致。PR 不允许在中间状态合入，因为部分合入会导致 scope-assignment 输出 `admin-panel` 但 init-justfile 仍期望 `frontend` 的不一致。

**过渡期兼容层**：阶段 1 的 `resolveScope()` 保留一个版本的向后兼容逻辑——若 scope 值为旧枚举（`frontend`/`backend`），在 surfaces map 中查找对应的 surface key 并映射（`frontend` → type=web 的 key，`backend` → type=api 的 key）。若 surfaces map 中不存在对应类型（如 `frontend` 映射 type=web 但 surfaces 中无 type=web 条目），输出警告并回退 scope 为 `all`，确保任何配置下都有确定性行为。

**多 surface 同类型冲突解决规则**：当多个 surface 同类型时（如两个 type=web 的 surface `admin-panel` + `marketing-site`），兼容层按以下规则消歧：(1) 若只有一个匹配类型的 surface，直接映射；(2) 若有多个匹配，**不依赖声明顺序**——Go 的 `map[string]string` 迭代顺序不确定，且 `SurfacesMap` 的 `UnmarshalYAML` 虽然通过 `yaml.Node.Content` 按插入顺序读取，但存储到底层 `map[string]string` 后顺序丢失。消歧策略改为：按 key 的**字典序**选择第一个匹配的 surface key（确定性且不依赖运行时行为）；(3) 同时输出警告："scope 'frontend' 映射到 'X'，但存在多个 type=web surface（X, Y），建议显式使用 surface key"。此兼容层在 **v3.1.0** 中移除（即保留一个 minor version：v3.0.x 全系列包含兼容层，v3.1.0 起移除）。这确保了如果某个组件在迁移窗口中遗漏，旧值仍能被正确映射而非静默丢弃。移除时需同步清理 `resolveScope()` 中的旧枚举映射代码，并在 CHANGELOG 中标注 breaking change。
...
**probe 进度输出**：probe 配方体每次重试输出 `[probe] [retry N/M] <target> — <result>` 到 stderr，使用户在等待过程中看到实时进度。检测到端口冲突时立即输出 `[probe] [abort] 端口冲突` 并以 exit 1 退出，提供早期退出条件。
...
2. **PID 有效性校验 + 命令行匹配 + 端口反查**（防止 PID 回收杀错进程，覆盖 npm wrapper 退出场景）：
   ...
   - **混合项目**：每 scope 独立 PID 文件（`.forge/dev-server.<scope>.pid`），teardown 逆序处理
...
- **journey 过滤策略最小规范**（每个 surface 规则文件必须定义）：
  ...
  **参数解析优先级**：`just test` 配方的参数签名为 `just test [scope] [journey]`。当只传入一个位置参数时（如 `just test admin-panel`），按以下优先级解析：(1) 先与 `config.yaml` 中 `surfaces` map 的 key 集合匹配——若匹配成功，视为 scope 参数，journey 为空（运行该 scope 的全部测试）；(2) 若不匹配任何 surface key，视为 journey 参数，scope 为空（运行所有 surface 的该类型测试）。当传入两个位置参数时（如 `just test admin-panel e2e`），第一个为 scope，第二个为 journey。此解析规则确保 `just test admin-panel e2e` 中 `admin-panel` 无歧义地被解析为 scope（因为它是 surfaces map 的 key），而 `just test e2e` 中 `e2e` 无歧义地被解析为 journey（因为它不在 surfaces map 的 key 集合中）。
