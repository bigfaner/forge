...
**关键映射**：`surface-key` 由 `surfaces map key` 经 `/` → `+` 转换而来。当 map key 不含 `/` 时两者相同（如 `admin-panel` → `admin-panel`）；当 map key 含 `/` 时（如 `admin/service`），surface-key 为 `admin+service`。选择 `+` 而非 `-` 是因为 `-` 在目录名中太常见，导致无法区分 `admin-panel`（原名含 `-`）和 `admin/panel`（`/` 转换为 `-` 后碰撞）。`+` 在实际项目目录名中极罕见，shell 无特殊语义无需转义。配方体内部需要 surface-key → surfaces map key 的反向查找时，通过匹配转换规则实现（`admin+service` → 查 config 找到 key `admin/service`）。

**surfaces map key 约束**：map key 不应包含 `+` 字符。若 map key 包含 `+`（如 `admin+panel`），转换后 surface-key 为 `admin+panel`，与 `admin/panel` 转换后的 `admin+panel` 碰撞。`forge init` 或 `init-justfile` 在读取 config.yaml 时检测到 `+` 应输出警告："surfaces map key 包含 '+' 字符，可能与 '/' → '+' 转换规则冲突，建议使用 '-' 代替 '+'"。
...
- **顺序启动策略**：`just dev` 配方体按顺序启动各 surface-key 的 dev server（先启动后端，再启动前端），每个启动后将 `{"pid": <pid>, "port": <port>}` 写入 `tests/{surface-key}/.dev-server.json`。顺序启动（而非并行）是为了避免多进程同时争抢系统资源导致启动失败
- **probe 重试差异化**：差异化重试在 `just probe` 配方体内部实现（而非 run-tests 层面），通过特定退出码向 run-tests 传递失败类型。配方体内区分三种失败模式：(1) **连接被拒绝**（ECONNREFUSED）— 端口无进程监听，继续重试（默认行为）；(2) **连接超时** — 端口有进程但不响应 HTTP，配方体内部增加计数权重（每次超时计为 2 次重试）加速退出，最终以 exit 3 退出（run-tests 识别后跳过后续 probe 直接 teardown）；(3) **端口冲突**（日志 EADDRINUSE）— 立即以 exit 2 退出（run-tests 识别后跳过所有重试直接 teardown）。退出码约定：exit 0 = 健康，exit 1 = 通用失败（服务未就绪，重试耗尽），exit 2 = 端口冲突（立即中止），exit 3 = 连接超时（加速退出）。run-tests 根据退出码执行对应的后续动作，无需解析 probe 的 stderr 输出
...
**最小通用 schema**（所有 surface 类型的 config.yaml 必须包含）：

```yaml
# tests/{surface-key}/config.yaml 最小通用字段
surfaceType: web          # 必填，surface 类型枚举
```

`surfaceType` 是唯一的必填字段，用于 testkit 校验配置与实际 surface 的一致性。其余字段为可选，由各 surface 规则文件按需定义。testkit 读取 `surfaceType` 后加载对应 surface 规则文件做字段校验——规则文件定义的必填字段若缺失，testkit 输出明确的错误信息（如 "web surface 的 config.yaml 缺少必填字段 baseUrl"）。

| Surface | 最小通用字段 | Surface 特有配置项（由规则文件定义） |
|---------|------------|--------------------------------|
| **web** | `surfaceType` | `baseUrl`、`apiBaseUrl`、`username`/`password`、页面 locators |
| **api** | `surfaceType` | `baseUrl`、`authToken`、`defaultHeaders` |
| **cli** | `surfaceType` | `binaryPath`、`envVars` |
| **tui** | `surfaceType` | `binaryPath`、`terminalSize` |
| **mobile** | `surfaceType` | `appPath`、`deviceName`、`platformVersion` |
...
```
获取 surface 信息（优先任务文档 → forge surfaces CLI 回退）
...
1. **获取 surface 信息**：**优先从当前任务的 frontmatter 读取 `surface-key` 和 `surface-type` 字段**（任务由 breakdown-tasks/quick-tasks 生成时已填充此信息）。若任务文档无此信息，通过 `forge surfaces` CLI 回退获取（CLI 内部读取 config.yaml 的 `surfaces` 字段，缺失时基于文件信号检测）
...
3. 更新 `forge-cli/pkg/forgeconfig/detect.go` 和 `forge-cli/internal/cmd/surfaces.go` 中的 `KnownSurfaceTypes` 白名单，使 `forge surfaces` CLI 识别新类型

**Go 代码校验验证**：Go 的 `surfaces` 字段类型为 `map[string]string`，结构体本身不做枚举校验。但 `forge-cli/pkg/forgeconfig/detect.go` 和 `forge-cli/internal/cmd/surfaces.go` 中各维护了一份 `KnownSurfaceTypes` 白名单——`ValidateSurfaceTypes()` 对未知类型输出警告，`SurfaceTypes()` 和 `forge surfaces --types` 直接跳过未知类型。因此新增 surface 类型**必须同步更新两个 Go 文件中的白名单**，否则新增类型会被系统静默忽略。
...
| 参数来源 | `project-type` 字段 + 文件路径模式匹配 | surface-key 由 `forge surfaces <path>` CLI 动态查询结果转换而来 |
...
| 组件 | 当前行为 | 迁移变更 |
|------|---------|---------|
| `breakdown-tasks` `rules/surface-key-assignment.md` | 文件路径模式：`ui/`/`components/` → frontend，`cmd/`/`internal/` → backend | 通过 `forge surfaces <path>` CLI 动态查询：文件属于哪个 surface → 该 surface-key |
| `quick-tasks` SKILL.md | 内联推断：UI → frontend，API → backend | 通过 `forge surfaces <path>` CLI 动态推断 surface-key |
...
| `prompt.go` `resolveScope()` | 按 `project-type` 枚举校验（backend 项目清 frontend） | **完全重写**为按 surface-key 集合校验：值不在集合中且非 `all`/空则清除。当前函数基于 `projectType` 硬编码 switch-case（只处理 `backend`/`frontend`），重写后切换为读取 `config.yaml` 的 `surfaces` 字段做集合查询 |
...
type Task struct {
    // ... 现有字段 ...
    SurfaceKey   string `json:"surface-key,omitempty"`   // 新增字段，值为 surface-key 或 "all"
    SurfaceType  string `json:"surface-type,omitempty"`  // 新增字段，surface 类型枚举：web/api/cli/tui/mobile
    Scope        string `json:"scope,omitempty"`          // 保留旧字段用于向后兼容（v3.1.0 移除）
}

// GetSurfaceKey 返回有效的 surface-key：优先 surface-key 字段，回退 scope 字段
func (t *Task) GetSurfaceKey() string {
    if t.SurfaceKey != "" {
        return t.SurfaceKey
    }
    return t.Scope // 兼容层：旧任务的 scope 字段
}
```

**双字段 JSON 兼容方案**：写入时同步写入 `scope` 和 `surface-key`（值相同），读取时通过 `GetSurfaceKey()` 统一访问。过渡期 index.json 示例：
...
v3.1.0 移除兼容层时：删除 `Scope` 字段和 `scope` JSON 键，`GetSurfaceKey()` 简化为直接返回 `SurfaceKey`，并删除 index.json 中所有 `scope` 键。
...
| 字段 | 填充时机 | 填充方式 |
|------|---------|---------|
| `surface-key` | `breakdown-tasks` / `quick-tasks` 生成任务时 | 通过 `forge surfaces <path>` CLI 查询受影响文件所属 surface（同现有 scope 推断时机，查询方式变更） |
...
| quality-gate fix-task | surface-key 或 `all` | web/api/cli/tui/mobile | 复用重写后的 `resolveScope()` 从失败文件路径推断；surface-type 从 surfaces map 查类型。无法推断时默认空 |
| auto-gen test tasks | 硬编码 `"all"` | 从任务类型提取 | 现有 `scope: "all"` 改为 `surface-key: "all"`；`TestType` 重命名为 `SurfaceType`（per-type 任务值不变，如 `"api"`） |
...
| `run-tests` | **优先从任务文档获取 `surface-key`/`surface-type`**（读取 task frontmatter 字段）；若任务文档无此信息（如手动创建的任务），回退到 `forge surfaces` CLI 或直接读取 `.forge/config.yaml` 的 `surfaces` 字段。此回退路径确保任何任务都能获取 surface 信息 |
| `fix-bug` | 验证修复后选择正确的测试命令 |

**surface-key-assignment 迁移细则**：

当前规则按目录名模式推断（`ui/` → frontend，`cmd/` → backend）。迁移后改为通过 `forge surfaces <path>` CLI 动态推断：

1. 对任务的每个受影响文件，调用 `forge surfaces <file-path>` 获取该路径所属的 surface（CLI 使用 longest-prefix-match 路由）
2. 所有文件属于同一 surface → surface-key 为该 surface 的转换值
3. 文件跨越多个 surface → surface-key 为 `all`
4. CLI 无匹配结果或 surfaces 配置缺失 → surface-key 为 `all`
5. 若同一文件的 CLI 调用返回多个 surface（歧义场景）→ surface-key 为 `all`

**关键数据源说明**：`surfaces` map 的 key 是用户自定义标识符（如 `admin-panel`），不是目录路径。surface → 目录路径的映射由 `forge surfaces` CLI 内部的路由逻辑处理（结合 `sources` map 和 longest-prefix-match）。scope-assignment 规则不直接解析 surfaces map 的 key 作为路径，而是委托给 CLI 做路径查询。

**跨平台路径规范化**：`forge surfaces <path>` CLI 内部对输入路径做统一规范化处理，确保跨平台一致性：

1. **分隔符统一**：输入路径中的 `\`（Windows）统一替换为 `/`后再做 longest-prefix-match 匹配。Go 的 `filepath.ToSlash()` 可实现此转换，且无跨平台兼容问题（纯字符串操作，不依赖 OS）
2. **相对路径基准**：输入路径相对于项目根目录（与 config.yaml 中 `sources` map 的路径语义一致）。CLI 接受绝对路径和相对路径，内部通过 `filepath.Rel(projectRoot, inputPath)` 统一转为项目根相对路径后再做匹配
3. **大小写处理**：路径匹配区分大小写（Linux/macOS）或不区分（Windows）。CLI 使用 `filepath.Match()` 或 `strings.EqualFold()`（仅 Windows）适配平台行为，与 Go 标准 `os` 包的文件系统语义保持一致。不建议在 CLI 层强制统一大小写行为——这会破坏 Linux 的路径语义
4. **调用方无需关心分隔符**：scope-assignment 规则（LLM agent 执行）和 SKILL.md 中的指令直接传递原始文件路径（可能含 `\`），CLI 内部处理规范化——调用方不需要预处理路径

**Shell 转义说明**：LLM agent 通过 Bash 工具调用 `forge surfaces src/ui/component.tsx`，路径由 shell 直接传递给进程参数（`os.Args[1]`），不涉及 shell 转义。Windows 上 git bash 环境（Forge 的标准运行环境）同样使用 `/` 分隔符，与 Linux 行为一致。
...
1. **阶段 1——数据模型（最先变更）**：`prompt.go` 中的 `resolveScope()` 函数**完全重写**——当前实现基于 `projectType` 硬编码 switch-case（只对 `backend`/`frontend` 做匹配），重写后切换为读取 `config.yaml` 的 `surfaces` 字段做集合查询。此变更不影响无 surfaces 配置的项目（集合查询无匹配时回退 `all`），但为后续组件提供正确的 surface-key 值域基础
2. **阶段 2——规则引擎**：`breakdown-tasks rules/surface-key-assignment.md` 和 `rules/db-schema.md`、`quick-tasks SKILL.md` 同步更新，surface-key 推断逻辑切换到 `forge surfaces <path>` CLI 动态查询
...
**过渡期兼容层**：阶段 1 重写的 `resolveScope()` 保留一个版本的向后兼容逻辑。**分两层处理**：

1. **无 surfaces 配置的项目**（`ReadSurfaces` 返回 nil）：**保留原行为**——`resolveScope()` 原样返回 `taskScope`，不做任何映射或清除。这确保纯语言项目（无 surface 概念）的行为完全不变，prompt 模板中的 `{{SCOPE}}` 继续传递 `frontend`/`backend`/空值
2. **有 surfaces 配置的项目**：若值为旧枚举（`frontend`/`backend`），在 surfaces map 中查找对应的 surface-key 并映射。若 surfaces 中不存在对应类型，输出警告并回退为 `all`

此分层设计避免了无 surfaces 项目中的行为回归——当前 `resolveScope()` 对 `backend` projectType 保留 `backend` scope，重写后无 surfaces 时不改变此行为。

**多 surface 同类型冲突解决规则**：当多个 surface 同类型时（如两个 type=web 的 surface `admin-panel` + `marketing-site`），兼容层按以下规则处理：(1) 若只有一个匹配类型的 surface，直接映射；(2) 若有多个匹配，**报错中止**——输出错误信息："'frontend' 无法自动映射：存在多个 type=web surface（admin-panel, marketing-site），请显式使用 surface-key 更新任务配置"。此策略比静默映射更安全——静默映射到错误的 surface 会导致任务在错误的上下文中执行，而报错中止强制用户明确指定，消除歧义。此兼容层在 **v3.1.0** 中移除（即保留一个 minor version：v3.0.x 全系列包含兼容层，v3.1.0 起移除）。移除时需同步清理 `resolveScope()` 中的旧枚举映射代码，并在 CHANGELOG 中标注 breaking change。
...
- Surface 信息来自任务文档（`surface-key`/`surface-type` frontmatter 字段）或 `.forge/config.yaml` 的 `surfaces` 字段或 `forge surfaces` CLI
- **Surface 信息源优先级规则**（init-justfile 和 run-tests 统一遵循）：
  1. **任务文档优先**：若当前操作有任务文档（task frontmatter），优先读取其 `surface-key` 和 `surface-type` 字段（由 breakdown-tasks/quick-tasks 生成时已填充）
  2. **`config.yaml` 的 `surfaces` 字段回退**：若任务文档无此信息，直接读取 `.forge/config.yaml` 的 `surfaces` 字段获取 surface-key → surface-type 映射
  3. **`forge surfaces` CLI 最终回退**：若 `surfaces` 字段缺失或为空，通过 `forge surfaces [path]` CLI 基于文件信号检测获取 surface 类型（**待验证**：需在实现时确认 CLI 返回格式）
  4. **冲突处理**：当 config 和文件信号检测不一致时，以 `config.yaml` 为准（用户显式配置优先于自动检测）
- Surface 规则保持语言无关
...
| NFR | 要求 | 验证方式 |
|-----|------|---------|
| **跨平台兼容** | Windows/macOS/Linux 三平台均可运行编排序列。`just dev`/`probe`/`test-teardown` 配方体需包含平台分支；`forge surfaces <path>` CLI 内部统一路径分隔符（`\` → `/`），调用方无需预处理 | 各平台手动验证；CI 矩阵（如果接入）；Windows 路径含 `\` 和空格的边界测试 |
...
| 测试后强制清理 | Cypress start-server-and-test | `just test-teardown` + `tests/.dev-server.json` 状态追踪 + `.forge/test-state.json` 异常恢复 |
...
```just
# 单 surface 项目示例；多 surface 项目路径为 tests/{surface-key}/
[linux]
dev:
    mkdir -p tests && nohup npm run dev > tests/.dev-server.log 2>&1 & echo "{\"pid\":$!,\"port\":$PORT}" > tests/.dev-server.json

[windows]
dev:
    powershell -Command "if (!(Test-Path 'tests')) { New-Item -ItemType Directory -Path 'tests' | Out-Null }; $p = Start-Process -FilePath 'npm' -ArgumentList 'run','dev' -RedirectStandardOutput 'tests\\.dev-server.log' -RedirectStandardError 'tests\\.dev-server-err.log' -PassThru -WindowStyle Hidden; \"{\\\"pid\\\":$($p.Id),\\\"port\\\":$PORT}\" | Set-Content -Path 'tests\\.dev-server.json' -Encoding utf8"
```

> **Dev server 状态文件**：`just dev` 启动后将 PID 和端口号写入 `tests/.dev-server.json`（JSON 格式 `{"pid":12345,"port":3000}`），teardown 从同一文件读取。多 surface 项目按 surface-key 隔离：`tests/{surface-key}/.dev-server.json`。文件放在 `tests/` 目录下因为 dev server 是为测试编排启动的，不属于持久化构建产物。
>
> **日志输出保留**：dev server 的 stdout/stderr 重定向到 `tests/.dev-server.log`（非 /dev/null），启动失败时（编译错误、依赖缺失）用户和 run-tests 可查看日志定位原因。probe 超时后 run-tests 在错误消息中附带日志最后 10 行内容，避免用户在"服务启动超时"后还需手动查找失败原因。
...
后台进程的 PID 和端口需要持久化以供 teardown 和端口反查使用：

- `just dev` 将 `{"pid": <pid>, "port": <port>}` 写入 `tests/.dev-server.json`（单 surface）或 `tests/{surface-key}/.dev-server.json`（多 surface）
- `just probe` 不需要 PID——它只检查 HTTP 端点可达性
- `just test-teardown` 读取 `tests/.dev-server.json`，解析 PID 和端口，先校验 PID 有效性，校验失败时用端口反查
- run-tests 在 `.forge/test-state.json` 中记录 teardown 命令（现有机制），同时记录状态文件路径用于异常恢复
...
probe:
    ...
    while [ $retries -lt $max ]; do
        ...
        if grep -q "EADDRINUSE" tests/.dev-server.log 2>/dev/null; then
...
probe:
    ...
    while ($retries -lt $max) {
        ...
        if (Test-Path "tests\.dev-server.log" -and (Select-String -Path "tests\.dev-server.log" -Pattern "EADDRINUSE" -ErrorAction SilentlyContinue)) {
...
3. **日志尾部检查**：检查 `tests/.dev-server.log` 最后 5 行是否含 `EADDRINUSE` 等错误模式，匹配则立即退出报告端口冲突
...
1. **正常路径**：`just test-teardown` 读取 `tests/.dev-server.json`（或 `tests/{surface-key}/.dev-server.json`），解析 PID 和端口，**先校验 PID 有效性和命令行匹配**，再向对应进程发送 SIGTERM/taskkill
...
2. **PID 有效性校验 + 命令行匹配 + 端口反查**（防止 PID 回收杀错进程，覆盖 npm wrapper 退出场景）：
   ...
   - **端口反查**（PID 校验失败时触发）：从状态文件的 `port` 字段获取端口号，通过监听端口定位实际进程 PID——Linux `lsof -i :$PORT -t`/`ss -tlnp`；macOS `lsof -i :$PORT -t`；Windows `netstat -ano | findstr :$PORT`。反查 PID 替代原 PID 作为终止目标。端口号由 `just dev` 启动时写入状态文件，teardown 从同一文件读取，无需环境变量解析或日志匹配
   - PID 无效且端口反查无结果时跳过终止，仅清理状态文件
   - **混合项目**：每 surface-key 独立状态文件（`tests/{surface-key}/.dev-server.json`），teardown 逆序处理
...
5. **状态文件清理**：teardown 完成后（无论 PID 有效与否）删除 `tests/.dev-server.json`（或 `tests/{surface-key}/.dev-server.json`）
...
- **运行时强制保障**：退出码是唯一的运行时确定性保障——每个步骤的退出码由 just 命令客观返回，LLM 无法篡改或忽略。在此基础上增加两层辅助机制：(1) **test-state.json 异常恢复**——run-tests 每步将 `{phase, status, pid_file}` 写入状态文件，用于会话中断后的清理恢复（如用户 Ctrl+C 后重启 run-tests），**不作为运行时步骤门控**——步骤间的逻辑依赖由退出码和 SKILL.md 指令共同保证；(2) **teardown 幂等性**——PID 不存在时跳过，违反 HARD-GATE 执行不必要的 teardown 也不产生副作用
...
- **surface-key 统一迁移**（见"维度 2"迁移影响面）：surface-key-assignment 规则更新、quick-tasks surface-key 推断更新、db-schema 规则更新、prompt.go resolveScope() 完全重写、init-justfile 混合项目配方生成更新

预计 25-30 个编码任务。

**PR 分阶段审查策略**：scope 迁移和 surface 感知编排在同一 PR 中完成，但按逻辑拆分为多个提交便于分阶段审查：

1. **提交 1——数据模型 + Go 白名单**：`prompt.go resolveScope()` 完全重写、`task/types.go` Scope→SurfaceKey、`autogen.go` 适配、`KnownSurfaceTypes` 更新（如有新类型）。此提交通过单元测试验证
2. **提交 2——规则引擎 + 模板层**：`surface-key-assignment.md`、`db-schema.md`、`quick-tasks` SKILL.md、`init-justfile` 混合项目配方、16 个 prompt 模板。此提交通过集成测试验证（生成任务并检查 surface-key 字段）
3. **提交 3——Surface 感知编排**：5 个 surface 规则文件、`run-tests` SKILL.md 重写、`init-justfile` SKILL.md 更新。此提交通过端到端验证（web/api 项目完整编排流程）
4. **提交 4——Config schema 变更**：移除 `test.execution` 文档、未知字段处理、per-surface config.yaml 定义

每个提交有独立的验证点，reviewer 可按提交逐阶段审查。PR 不允许在中间状态合入（CI 在 PR 级别验证所有提交的联合行为）。
...
审计通过 `grep -r "test.execution" plugins/forge/` 执行（覆盖 skills/、references/、templates/ 等所有子目录）。若发现除 `run-tests` 外的引用，需在对应文件中同步更新为 `just test` 调用路径，作为 surface-key 统一迁移的附加任务。
...
- **journey 过滤策略最小规范**（每个 surface 规则文件必须定义）：
  ...
  **参数解析优先级**（仅多 surface 项目适用）：多 surface 项目的 `test` 配方签名为 `just test [surface-key] [journey]`，just 按位置绑定参数。当只传入一个位置参数时（如 `just test admin-panel`），just 将其绑定为 surface-key——配方体需做反转判断：(1) 先与所有 surface-key 集合匹配——若匹配成功，视为 surface-key 参数，journey 为空（运行该 surface-key 的全部测试）；(2) 若不匹配任何 surface-key，视为 journey 参数，surface-key 为空（运行所有 surface 的该类型测试）。当传入两个位置参数时（如 `just test admin-panel e2e`），第一个为 surface-key，第二个为 journey。**单 surface 项目**签名为 `just test [journey]`，无反转逻辑——参数直接绑定为 journey。
  ...
  > **反转逻辑的实现依赖**：配方体需要知道当前项目的 surface-key 集合才能执行反转判断。surface-key 集合的获取方式：(1) init-justfile 生成时将 surface-key 列表硬编码到配方体中（如 `KNOWN_KEYS="admin-panel payment-service"`），优点是无运行时依赖，缺点是 surfaces 配置变更后需重新运行 init-justfile；(2) 配方体运行时通过 `forge surfaces` CLI 动态获取，优点是始终与配置同步，缺点是引入 CLI 依赖和运行时开销。推荐方案 1（硬编码），因为 surface 配置变更频率低且 `init-justfile` 本身就是配置变更后的重生成入口。若 `# user-customized` 保护导致配方未更新，反转逻辑使用过期的 surface-key 集合——此时最坏情况是将新的 surface-key 误判为 journey 名称，导致运行所有 surface 的匹配 journey 测试（行为降级但不报错）。
...
- `breakdown-tasks` `rules/surface-key-assignment.md`：文件路径分类改为通过 `forge surfaces <path>` CLI 动态查询（见"维度 2"迁移细则）
...
- `prompt.go` `resolveScope()`：**完全重写**——从 `projectType` 硬编码 switch-case 切换为 surfaces map 集合查询
- `init-justfile` SKILL.md：混合项目所有配方的 surface-key case 分支改为 surface-key
- **Task Go struct**：`Scope` 字段重命名为 `SurfaceKey`，新增 `SurfaceType` 字段（接收 `AutoGenTaskDef.SurfaceType` 值，即原 `TestType` 重命名后的字段）；旧 `scope` JSON 键保留一个版本用于兼容（v3.1.0 移除）
...
- **`forge task add` CLI**：`AddTaskOpts` struct 新增 `SurfaceKey` 和 `SurfaceType` 字段。当 `SourceTaskID` 非空时，从源任务的 `GetSurfaceKey()` 和 `SurfaceType` 复制到新任务。无源任务时：单 surface 项目 surface-key 为空（surface-type 填充唯一 surface 的类型），多 surface 项目均默认空。**不新增 CLI flag**——继承优先于手动指定
- **quality-gate fix-task**：`addFixTask()` 需新增 surface-key/surface-type 填充逻辑。当前 `AddTaskOpts` struct 无 `Scope` 字段，迁移时需新增 `SurfaceKey`/`SurfaceType` 字段。推断方式：从 `extractSourceFiles(output)` 提取的源文件路径中取第一个文件，调用 `forge surfaces <path>` CLI 获取 surface-key，再查 config.yaml 的 surfaces map 获取 surface-type。注意：当前 `addFixTask()` 不调用 `resolveScope()`——scope 的处理发生在 `renderTemplate()` 中的 `{{SCOPE}}` 替换阶段（`prompt.go:137`）。迁移后 `AddTaskOpts.SurfaceKey` 在 `addFixTask()` 中直接填充，`renderTemplate()` 改为读 `t.GetSurfaceKey()`。无法推断时默认空
- **auto-gen test tasks**：`autogen.go` 中 `AutoGenTaskDef.Scope` 重命名为 `SurfaceKey`（保持 `"all"`），**`TestType` 字段重命名为 `SurfaceType`**（per-type 任务值不变，如 `T-test-gen-journeys-api` → `"api"`）。`TestType` 可安全移除——经代码审计确认：(1) per-type 任务中 `TestType` 和 `SurfaceType` 值始终相同（都等于循环变量 `typ`），非 per-type 任务两者都为空，零差异场景；(2) `extractTestTypeArg()`（prompt.go:307）从任务 ID 字符串提取后缀（`ExtractTypeSuffix("T-test-gen-scripts-api", "T-test-gen-scripts")` → `"api"`），不读 `TestType` 字段，移除不影响 `{{TEST_TYPE_ARG}}` 注入；(3) `GenerateTestTaskMD()` 中 `def.TestType` 的 4 处引用（frontmatter `{{TEST_TYPE}}` 替换 + body `Type: **api**` 展示）全部改为 `def.SurfaceType` 即可

**Autogen 传播链修改方案**（`AutoGenTaskDef` → `Task` struct → index.json 的完整路径）：

| 传播点 | 文件 | 当前代码 | 修改 |
|--------|------|---------|------|
| struct 定义 | `autogen.go` AutoGenTaskDef | `Scope string`、`TestType string` | `Scope` → `SurfaceKey string`；`TestType` → `SurfaceType string`（重命名，非新增） |
| Task 构建 | `autogen.go` TaskFromFile() | `Scope: d.Scope` | 新增 `SurfaceKey: d.SurfaceKey`、`SurfaceType: d.SurfaceType`；兼容期保留 `Scope: d.SurfaceKey` |
| frontmatter 写入 | `autogen.go` GenerateTestTaskMD() | `scope: "all"` | 双写 `scope: "all"` + `surface-key: "all"`；新增 `surface-type` 行 |
| frontmatter 解析 | `build.go` ParseFrontmatter() 返回的 struct | `Scope string` | 新增 `SurfaceKey string`、`SurfaceType string` 字段 |
| BuildIndex Task 构建 | `build.go:119` | `Scope: fm.Scope` | 新增 `SurfaceKey: fm.SurfaceKey`、`SurfaceType: fm.SurfaceType`；兼容期 `Scope: fm.SurfaceKey` |
| prompt 渲染 | `prompt.go:121` renderTemplate() | `resolveScope(opts.ProjectRoot, t.Scope)` | 改为 `resolveScope(opts.ProjectRoot, t.GetSurfaceKey())`；同时 `extractTestTypeArg(t.ID)` 改为直接读 `t.SurfaceType`，删除 `extractTestTypeArg()` 函数和 `genScriptBases` 列表 |

**注意**：`BodyContext.Scope`（planning-time 的 "In Scope" 列表，来自 proposal/PRD 文档）与 `Task.Scope`/`SurfaceKey`（任务 surface 归属）语义完全不同，前者是 `[]string` 类型，后者是 `string` 类型。两者在不同模板系统中使用，不直接冲突，但后续维护者需注意区分。
...
### 附带清理（Dead Code Removal）

提案迁移 `TestType` → `SurfaceType` 后，以下代码变为死代码可一并删除：

| 删除项 | 文件 | 原因 |
|--------|------|------|
| `extractTestTypeArg()` 函数 | `prompt.go:305-314` | `{{TEST_TYPE_ARG}}` 改为直接读 `t.SurfaceType`，无需从任务 ID 字符串解析 |
| `genScriptBases` 变量 | `prompt.go:301-303` | `extractTestTypeArg` 的唯一消费者，随函数一起删除 |
| `ExtractTypeSuffix()` 在 `extractTestTypeArg` 中的调用 | `infer.go:70` | 函数本身保留（`TaskFromFile()` 等其他路径仍在使用），但 `extractTestTypeArg` 不再调用它 |
...
| run-tests 无法感知 surface（任务文档无 surface-key/surface-type） | 低 | 高 | run-tests 优先从任务文档获取 surface-key/surface-type；缺失时回退到 `forge surfaces` CLI 或 `.forge/config.yaml` |
...
- [ ] surface-key 值域统一迁移：混合项目所有配方的 surface-key 参数值从 `frontend`/`backend` 迁移为 surface-key，surface-key-assignment、prompt.go resolveScope() 完全重写、init-justfile 配方生成同步更新
- [ ] Task 数据模型扩展验证：Go struct `Scope` → `SurfaceKey`；`AutoGenTaskDef.TestType` → `SurfaceType`（重命名，非新增）；`Task` struct 新增 `SurfaceType` 字段；任务模板 frontmatter 包含 `surface-key` 和 `surface-type`；`breakdown-tasks` / `quick-tasks` 生成的任务中两个字段正确填充；`forge task add` 从源任务继承 `surface-key` 和 `surface-type`（无源任务时单 surface 项目 surface-key 为空，不新增 CLI flag）；quality-gate fix-task 从失败文件路径自动推断 surface-key/type；auto-gen test tasks 的 `SurfaceType` 从 per-type 定义提取（原 `TestType` 值）；旧任务的 `scope` 字段通过兼容层正确映射
...
- [ ] config schema 变更验证：`.forge/config.yaml` 中无 `test.*` 配置项；用户 config.yaml 中残留的 `test.execution` 节点不导致 YAML 解析错误；`GetConfigValue` 对 `test.execution.*` 的查询返回明确的移除提示；SKILL.md 中无任何 timeout 参数配置；testkit 能正确解析 per-surface config 并向测试脚本暴露配置项（如 web 的 baseUrl/credentials、api 的 authToken 等）；testkit 读取 surfaceType 后校验与实际 surface 类型的一致性（不一致时报错）
