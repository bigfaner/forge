...
- **用户反复拒绝专家**：用户连续 3 次拒绝系统生成的专家档案 → 系统提示用户手动输入专家描述，或选择跳过自由评审阶段。注：此「3 次拒绝」计数器与「3 轮修改」计数器独立运作——修改不消耗拒绝次数，拒绝不消耗修改轮次
...
- **部分提取失败**：自由评审叙事包含 5 个风险点但提取仅成功 2 个（JSON 部分字段缺失或格式错误）→ 系统记录提取命中率（成功提取数 / 叙事中包含「风险」「问题」「隐患」「建议」等关键词的段落数。注：此为粗粒度启发式估算，在风险密集段落中会高估总数导致命中率偏低，在隐含风险表述中会低估总数导致命中率偏高。接受此不精确性，因为命中率的用途是触发告警而非精确度量——50% 的阈值足以捕获严重的提取失败），若命中率 < 50% 则在评审报告中标注「提取命中率低」，同时将完整自由评审叙事附加到评审报告供用户手动查阅
...
学术同行评审系统长期使用"领域匹配审稿人"机制——系统根据论文内容推荐具备相关背景的审稿人，审稿人以自由文本撰写评审意见，编辑从自由评审中提取决策依据。这一模式是学术出版的基础假设（尽管严格的 A/B 实证在学术界本身难以执行）：(1) 专家-内容匹配被普遍认为比固定角色更有可能覆盖领域特有问题（见 Key Risks 表中关于实证基础薄弱的风险条目），(2) 自由叙事评审能覆盖结构化检查表无法预设的问题。类比到提案评审：当前 eval-proposal 的固定 CTO 角色等同于所有论文都由同一位通用审稿人评审，缺少领域深度。
...
4. 发现提取 + 注入机制（含提取 prompt 模板、JSON 校验、命中率计算、降级逻辑）
...
- [ ] 专家修改流程正确工作：用户选择「修改专家描述」后，系统基于用户输入重新生成专家档案并再次呈现确认；最多 3 轮修改后系统提示接受或跳过
...
- [ ] 专家废弃机制可被触发和验证：对同一专家连续 3 次评审均无「实质变化」（rubric 评分差异 < 15 分且 attack points 列表未变动）时，该专家的 YAML front matter 中 `deprecated` 字段自动置为 `true`
