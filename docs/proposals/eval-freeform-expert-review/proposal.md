...
在 eval-proposal 的 rubric 循环之前，插入一个 **Phase 0 自由专家评审**阶段：

1. **动态专家生成**：分析提案内容（domain、技术栈、复杂度、关键决策），推断最适合评审的专家档案（背景、专业领域、评审风格），用户确认后使用
2. **自由叙事评审**：该专家以纯叙事形式对提案进行深度评审——无 rubric、无评分、无预设维度，完全由专家自主决定关注什么
3. **发现提取与注入**：从自由评审叙事中提取结构化发现（key findings），注入后续 rubric scorer 的 prompt 作为额外上下文
4. **专家持久化与复用**：动态专家档案保存到 `docs/experts/` 全局目录，后续评审可复用已有专家
...
完全可行。核心改动点：
1. eval skill 的 proposal 类型处理流程中增加 Phase 0
2. 新增一个「专家推断 + 自由评审」子 agent prompt
3. 新增发现提取逻辑
4. 修改 rubric scorer prompt 以接收注入的发现

所有改动均在现有 eval skill 架构内完成，无需新框架或外部依赖。

### Resource & Timeline

预计 3-5 个 coding task：
1. 专家推断逻辑 + 专家档案模板
2. 自由评审子 agent prompt + 协议
3. 发现提取 + 注入机制
4. eval skill 集成（Phase 0 编排）
5. 专家持久化与复用逻辑
...
- [ ] eval-proposal 执行时自动进入 Phase 0，生成动态专家档案并经用户确认
