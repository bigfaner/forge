...
4. **发现提取与注入**：通过 LLM 结构化输出提取自由评审叙事中的 key findings。具体流程：自由评审叙事 + 提取 prompt（指令：「从以下自由评审叙事中提取所有显式提出的风险点和改进建议。输出 JSON 数组，每个元素包含 summary（一句话概括）、severity（high/medium/low）、quote（叙事中的原文引用）字段。仅提取评审者明确表述的风险，不要推断隐含风险。」）→ LLM 提取 → 验证提取产出非空且 JSON 格式合法 → 验证每个元素三个字段均非空 → 注入 rubric scorer prompt。若提取产出为空或格式校验失败，降级为标准 rubric 流程。需坦诚承认：从自由叙事中提取结构化发现本身是一个有损的机械过程——叙事中的推理链条、语气强度、上下文关联在提取为 bullet points 时不可避免地丢失。本提案接受这一损失，理由是：(1) 提取的 target 是 rubric scorer 而非人类读者，scorer 需要的是明确的 attack points 而非细腻的叙事；(2) 完整的自由评审叙事会同步保存在评审产出中供用户查阅，提取仅作为注入 rubric 的中间产物。可接受的信息损失边界：提取的 key findings 必须覆盖自由评审叙事中所有显式标注的风险点（不含隐含推断），由于缺乏自动化的 ground truth 标注，遗漏率 ≤ 10% 为设计目标而非可自动验证的指标，需通过人工抽检验证
...
- **部分提取失败**：自由评审叙事包含 5 个风险点但提取仅成功 2 个（JSON 部分字段缺失或格式错误）→ 系统记录提取命中率（成功数/预估总数），若命中率 < 50% 则在评审报告中标注「提取命中率低」，同时将完整自由评审叙事附加到评审报告供用户手动查阅
