...
# Proposal: Task Type Prefix-Based Classification
...
`InferType` 仍为映射表（ID 后缀不等于类型名），但 key 从数字变为可读字符串。
...
**理由**: ID 后缀不完全等同于类型名（`gen-cases` ≠ `test.gen-cases`，`clean-code` ≠ `coding.simplify`）。映射表的 key 变得更可读，但逻辑结构不变。

### D4: `noTest` 保留作为显式覆盖

**选择**: `coding.*` 类型隐式走 quality-gate，`doc` / `test.*` / `gate` 隐式跳过。`noTest: true` 保留给 edge case（coding task 但不需要测试）。

## Requirements Analysis

### Key Scenarios

1. **纯 markdown feature**: 所有任务 `type: "doc"` → 自动跳过 quality-gate → `IsTestableType` 返回 false → 生成 `T-eval-doc`
...
- `forge-cli/pkg/task/infer.go` — 更新映射 key（数字 → 可读字符串）
...
- [ ] Go 类型常量全部使用 `coding.`/`doc.`/`test.` 前缀
