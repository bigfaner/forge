...
- Remove `claude`, `claude-c`, `claude-w` recipes from project justfile (`claude-w` is manually maintained, not generated by `forge init`)
- Update `forge init` to stop generating `claude` and `claude-c` justfile recipes
...
- [ ] `forge init` no longer generates `claude` and `claude-c` justfile recipes
