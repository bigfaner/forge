...
- 1 个 doc task：cli.md + tui.md + api.md 结构修正 + 守则补充 + 路径/引用修复
- 1 个 doc task：ui.md 修正 + mobile.md 重写（框架无关化）
- 1 个 doc task：SKILL.md 加载逻辑 + 优先级声明（类型规则加载在 Step 2 读取 Contract 之后、Step 3 生成代码之前）
- 总计 4 个 task
