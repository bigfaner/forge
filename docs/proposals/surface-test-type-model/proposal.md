...
测试执行时，justfile recipe 输出标签从 `Running e2e tests...` 变为 `Running CLI functional tests...` / `Running API functional tests...`。测试报告中的 suite 名称从 `e2e/journey-name` 变为 `cli-functional/journey-name`。CI dashboard 上每个测试类型的执行结果独立显示，不再混在单一的 "e2e" 分类下。
...
**Justfile alias 兼容**：`init-justfile` skill 当前使用 justfile 模板生成 recipe。实现方式：在每个 surface 的 justfile 模板中，新增 `alias old_name := new_name` 行（just 原生支持 alias 语法）。例如 CLI surface 的模板将包含 `alias test-e2e := test-cli-functional`。过渡期强制执行方式：alias 行带有 `# DEPRECATED: removed after v{version}` 注释，`init-justfile` skill 的版本号硬编码为当前 Forge 版本 +2，到达目标版本后从模板中移除 alias 行。用户重新运行 `forge init-justfile` 时自动更新。
...
3. **测试执行**：justfile recipe 按测试类型命名（如 `test-cli-functional`、`test-api-functional`），而非统一的 `test-e2e`
...
1. **向后兼容**：旧 justfile recipe 名（`test`、`test-e2e`）在过渡期内（2 个 Forge 版本）必须仍可执行，作为新 recipe 的 alias 存在
2. **迁移性能**：术语更新工作在单次 forge 执行周期内完成，不要求重新生成测试代码或重新运行测试
3. **可发现性**：用户通过 `just --list` 即可看到按测试类型命名的 recipe 列表，无需查阅外部文档即可理解每个 recipe 的用途
4. **测试执行性能零影响**：术语变更不引入新的测试执行步骤或额外的进程启动开销，每个 surface recipe 的执行路径与当前 `test-e2e` recipe 完全一致
5. **CI 集成无破坏性变更**：CI pipeline 中引用旧 recipe 名（`just test-e2e`）在过渡期内继续工作；CI 配置文件的更新不作为本提案的必要步骤，而是在过渡期结束前由各项目按需更新
6. **过渡期结束追踪**：alias 移除作为一个独立 task 记录在 Forge 版本规划中，由 init-justfile skill 的版本号硬编码机制自动触发，不依赖人工记忆

### Edge Cases

1. **多 surface 项目**：项目同时包含 CLI 和 API surface 时，justfile 中应包含 `test-cli-functional` 和 `test-api-functional` 两个独立 recipe，同时保留 `test` 作为运行所有测试的聚合 recipe
...
2. **迁移成本**：所有引用 "e2e" 的文件需逐一更新，现有项目已生成的 justfile 和测试代码不会自动迁移。缓解：向后兼容 alias 保证旧命令仍可用。
...
| justfile recipe 重命名影响已有项目的 CI 流程 | M | H | 提供向后兼容的 alias recipe（旧名 → 新名），设置 2 版本过渡期 |
...
- [ ] 旧 justfile recipe 名（`test`、`test-e2e`）作为 alias 仍可执行，无破坏性变更
- [ ] 测试执行输出中的 suite 名称和标签使用 surface-specific 测试类型名称（如 `cli-functional` 而非 `e2e`），质量门报告中不同测试类型的执行结果分类展示
