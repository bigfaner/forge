...
6. **Metadata frontmatter 驱动校验**：新增模板时声明 variables 列表，启动时自动校验模板数据 struct 与声明一致
...
- `template.go` 重构：移除 `ApplyVars()` 和 `injectSurfaceFrontmatter()`（含替换和插入两种行为），改为 `text/template`：约 1 小时
- `autogen.go` 重构：移除 `renderBody()` + `removeLineContaining()`/`removeSection()`，改为 `text/template`：约 2 小时
- Surface 硬性约束 + `{{SCOPE}}` 两种模式迁移 + `BodyContext.Scope` 替换为 `ScopeDisplay` 预格式化字段：约 1 小时
...
- 移除 `cleanTemplateOutput()` 中的全部四种条件删除逻辑（空标签行、空 backtick 条件句、`just` 命令尾部空白、`<!-- IF NOT_LOW -->...<!-- END_IF -->` 段落块）。函数保留但仅做空白行塌陷
...
- 模板须包含 `surface-key` 和 `surface-type` 字段行（当前 `injectSurfaceFrontmatter()` 同时执行替换已有字段和插入缺失字段两种行为，统一后模板必须预置这两个字段行）
...
- `{{SCOPE}}` 两种使用模式的迁移：
  - **段落级**（doc-consolidate, test-gen-contracts, test-gen-journeys, test-run）：`{{SCOPE}}` 独占一行或作为段落标题的一部分 → 整段用 `{{if .SurfaceKey}}...{{end}}` 包裹
  - **行内值**（其他使用点）：`{{SCOPE}}` 作为行内占位符 → 替换为 `{{.SurfaceKey}}`

- `BodyContext.Scope`（`[]string` 类型）替换为预格式化字符串字段：Go 代码中 `strings.Join(scopes, ", ")` 生成 `ScopeDisplay string`，模板直接输出 `{{.ScopeDisplay}}`，避免模板中 `{{range}}` 循环
...
- 模板数据 struct 定义：`autogenTemplateData`
  ...
  type autogenTemplateData struct {
      ...
      SurfaceType  string // surface type
      ScopeDisplay string // 替换原 BodyContext.Scope 的预格式化字符串（strings.Join(scopes, ", ")），供模板直接输出
  }
  ```


**renderTemplate() 中 TASK_CATEGORY 注入迁移**：
- `pkg/prompt/prompt.go` 第 136-137 行的 `TASK_CATEGORY` 字符串拼接逻辑迁移到模板数据字段
- 在 `promptTemplateData` 中增加 `TaskCategory string` 字段
...
- 在 `forge init` 中增加 surface 配置步骤（`forge surfaces detect` 集成），确保新项目初始化即具备 surfaces 配置，从源头避免硬性失败场景
...
| Surface 硬性失败阻塞无 surfaces 配置的项目 | M | H | 错误信息包含 `forge surfaces detect` 命令指引；`forge init` 中集成 surface 配置步骤（见 Scope 交付物） |
...
- [ ] `cleanTemplateOutput()` 仅保留空白行塌陷逻辑，移除全部四种条件删除逻辑（空标签行、空 backtick 条件句、`just` 命令尾部空白、`<!-- IF NOT_LOW -->...<!-- END_IF -->` 段落块）
...
- [ ] `BodyContext.Scope` 字段替换为预格式化字符串字段 `ScopeDisplay string`，模板中无 `{{range}}` 循环
...
- [ ] `ValidateTemplates()` 使用 `variables` 字段与模板数据 struct 做交叉校验
