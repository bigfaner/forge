...
| **AI 优先侧重** | Contract 为主(80%) | Contract 为主(80%) | 平衡(50/50) | Journey 烟测试为主(60-70%) | 平衡(50/50) |
| **Contract 核心价值** | 单命令 I/O 验证 | 单步交互+异步 Cmd 验证 | 单页面行为验证 | 单屏幕交互(deep link 入口) | 单端点 I/O 契约验证 |
| **Journey 烟测试核心价值** | 命令链状态传播 | 连续键盘流累积效果 | 跨页面完整用户体验 | 跨屏幕完整移动端流程 | API 链路完整业务流程 |
| **必须衍生的边界 Outcome** | `not-found` + `already-exists` | `timeout`(每个异步 Cmd) | `validation-error` + `session-expired` | `permission-denied`(每个权限操作) | `unauthorized`(每个认证端点) |
...
**关键设计建议**：gen-contracts 应基于场景类型差异化 Outcome 衍生策略，建议在 type 文件中增加 `required_outcomes` 声明，强制每个场景类型必须衍生的 Outcome 类别（如 API 强制 `unauthorized`、TUI 强制 `timeout`、Mobile 强制 `permission-denied`）。
...
- 合约规范增强：支持边界/异常场景自动衍生描述（排除纯函数逻辑分支等单元测试级别边界）
- 风险驱动测试密度：高风险旅程生成更密集的测试矩阵
- Contract 测试（集成层）+ Journey 烟测试（E2E 层）生成，按场景类型差异化侧重比例
- 场景差异化：按 CLI/TUI/WebUI/Mobile/API 定义策略差异、层级侧重、和必须衍生的边界 Outcome
