...
Forge Plugin v3.0.0-rc.53（22 个 skill、16 个命令、5 个 hook）经历大规模重构后，通过八维度深度审计（内部自洽性、Pipeline 契合度、提示词模板、任务模板、Surface 系统、Eval 系统、Hook 系统、Config 系统）发现了 23 处不一致问题，其中 4 个为 HIGH 级别静默错误——不会导致崩溃但会在用户执行 pipeline 时产出错误结果：
...
1. **数据同步**（H-1）: 更新 `rubric-reference.md` 表格 + `eval-journey`/`eval-contract` 的 `argument-hint` 和 `description` 字段。在 `rubric-reference.md` 头部添加维护注释，标明此文件为 rubric scale/target 的二级缓存
...
| INLINE 跨 skill 引用过时 | 中 | 中 | 为 3 处 INLINE 引用添加源文件版本号标记（如 `@ v3.0.0-rc.53`），便于 grep 检测过时引用 |
...
- **同时影响**: `commands/eval-journey.md` 和 `commands/eval-contract.md` 的 `argument-hint` 显示 `--target 850`，与实际 target 975/935 不符；两个命令文件的 `description` 字段仍声称 "1000-point rubric"，与实际 scale 1150/1100 不符，会误导 LLM 执行上下文
- **系统性风险**: eval 生态存在多个真相源（rubric frontmatter、rubric-reference.md、命令 argument-hint、命令 description、config 键默认值），缺乏单一真相源机制，未来变更需同步至少 5 处
- **修复**: 更新 rubric-reference.md 表格、两个命令文件的 argument-hint 和 description 字段；在 rubric-reference.md 头部添加维护注释，标明此文件为 rubric scale/target 的二级缓存，任何 rubric frontmatter 变更必须同步更新此文件
...
#### M-9: INLINE 跨 skill 引用同步风险（3 处，由 L-4 升级）
...
- [ ] H-1: eval-journey/eval-contract 的 argument-hint 和 description 字段均反映正确的 target/scale 值
...
- [ ] M-1: auto.eval 配置键统一为 kebab-case（前提：验证 Go config reader 是否支持 kebab-case 查询；如不支持，将 M-1 与 Go alias 绑定为原子操作，推迟至 Go 代码变更窗口执行）
...
1. **H-1**: 更新 rubric-reference.md + eval-journey/eval-contract argument-hint + description 字段（数据修正，无风险）
