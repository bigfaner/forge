...
1. **任务生成环节**：拆分规则从"时间估算"改为"可独立验证"标准；Reference Files 从 proposal section 指针改为内联精确信息；移除 quick-tasks 15 coding task 上限
2. **任务元数据环节**：task frontmatter 加 `complexity: low/medium/high` 字段，生成时自动判定
3. **执行环节**：prompt template 根据 complexity 字段分支（low 跳过 spec-code scan、简化探索）；加"先收集后修改"搜索策略引导
...
所有改动都在配置层（SKILL.md、template .md、prompt.go），不涉及运行时逻辑变更。`renderTemplate()` 已有 7 个占位符，加一个 `{{COMPLEXITY}}` 是最小扩展。
...
- quick-tasks SKILL.md 拆分规则优化：合并标准改为"可独立验证"、AC 上限 6 条、多动词检测规则、移除 15 coding task 上限
- breakdown-tasks SKILL.md 同步修改拆分规则
- task frontmatter 加 `complexity` 字段（quick-tasks 和 breakdown-tasks 的 task.md 模板）
- quick-tasks/breakdown-tasks 复杂度自动判定逻辑（AC 数量 + Hard Rules + Reference Files 数量）
- Reference Files 生成策略改为内联精确信息（quick-tasks 和 breakdown-tasks）
- 5 个 coding prompt templates 加复杂度分支（low 跳过 Step 1.5，简化 Step 1）和搜索策略引导
...
| prompt template 复杂度分支增加模板维护成本 | M | L | 分支逻辑统一模板化（一段条件块，5 个 template 复制相同结构） |
