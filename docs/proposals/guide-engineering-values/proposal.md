...
**S1: 新增价值观 preamble。** 在 `# Forge Guide` 标题之后、`## Directory Conventions` 之前，插入 `<ENGINEERING-VALUES>` 价值观层（~40 行）。基于 Karpathy 四原则，泛化命名（去掉 code-centric 用词），适用于 Forge 所有任务类型（编程、文档、设计、评估等）。原则名称精简、描述为可执行行为准则，无场景举例。
...
价值观层：采纳社区验证的 Karpathy 四原则核心思想，但做关键调整——泛化命名使其适用于所有任务类型（不仅编程），同时保持简洁（无场景举例，纯行为准则）。原则映射：
- "Think Before Coding" → "Think First"（行动前先推理）
- "Simplicity First" → 保持不变（通用）
- "Surgical Changes" → "Minimal Touch"（只动必要的）
- "Goal-Driven Execution" → 保持不变（通用）
...
- **Task executor 执行任务时**：价值观层指导 subagent 在动手前先理解任务、保持最小实现、只改必要内容、定义并验证成功标准
- **非编程任务时**（写 PRD、设计 UI、评估文档、整合 spec）：同样原则适用——先理解需求、保持简洁、精准修改、目标驱动
...
- 四原则泛化命名（去 code-centric 用词），适用于所有任务类型，无场景举例
...
| 原则过于抽象，agent 无法执行 | L | M | 每条原则为可执行行为准则，无抽象口号 |
...
- [ ] Preamble 包含四个泛化原则，适用于所有任务类型（无 code-centric 用词）
- [ ] Preamble 总行数 ≤ 40 行
