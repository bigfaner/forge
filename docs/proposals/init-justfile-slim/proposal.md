...
4. prompt 层（SKILL.md + 保留的 rules）总行数 <= 280 行
...
所有 recipe 均包含 `[unix]`（覆盖 Linux + macOS）和 `[windows]` 双平台变体。**标记策略**：所有 lifecycle recipes（dev/probe/test/teardown/`<key>`）和 quality recipes（compile/fmt/lint/unit-test）均标记 `# user-customized`，聚合 recipes（install/ci/clean）不标记。标记 `# user-customized` 的 recipe 在后续 `/init-justfile` 调用时被保护机制保留，不会被覆盖。
