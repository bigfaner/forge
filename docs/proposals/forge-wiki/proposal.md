...
| `forge kb search [--type] [--artifacts] [--concerns] [--langs] [--frameworks] <query>` | Pull | 多维度搜索，返回元数据列表（名称 + concerns + 描述） |
...
| `forge kb concerns` | Meta | 列出 wiki 中所有可用知识领域（concerns 值）及文档数量 |
...
```
## auth-jwt-decision
- type: product | concerns: [auth] | langs: [go] | frameworks: [gin]
- JWT Token 认证决策及理由
[来源: my-backend/docs/decisions/auth-jwt.md]

## auth-session-decision
- type: product | concerns: [auth] | langs: [go]
...
```bash
# Wiki 目录不存在或为空
$ forge kb search --concerns auth "token"
...
**Step 1 — 搜索（便宜）**：
```bash
forge kb search --type language --artifacts api --concerns auth --langs go "认证流程"
# → [{name: "auth-flow", type: "language", concerns: ["auth", "security"], desc: "..."},
#    {name: "session-mgmt", type: "product", concerns: ["auth"], desc: "..."}]
```
