...
- 删除 `forge-cli/internal/cmd/test_test.go` 和 `test_verify_test.go`（集成测试）
- 从 `testrunner` 包中删除 journey 隔离函数：`ResolveJourneyExecutionConfig`、`CreateJourneyWorkDir`、`ExecuteJourneyInIsolation` 及相关类型（`JourneyResult`、`ContractFailure`、`JourneyExecutionConfig`）
...
- 清理 skill 文档中的 `forge test` 引用：`run-tests`、`run-tasks`、`gen-contracts`、`gen-journeys`、`consolidate-specs`
- 清理 e2e 测试：`tests/command-regression/` 中 test 命令相关的测试用例和 contract spec
...
- [ ] `forge quality-gate` 和 `forge feature complete` 功能不受影响
