...
dependencies: ["T-review-doc", "1", "2", "3"]
type: "test.gen-journeys"
surface-key: ""
surface-type: ""
---

Generate test Journey documents for the test-pipeline-interleaved feature.
Mode: quick
...
- **Breakdown mode**: Read PRD user stories from `docs/features/test-pipeline-interleaved/prd/prd-user-stories.md` and functional specs from `docs/features/test-pipeline-interleaved/prd/prd-spec.md`. These are the primary input sources.
- **Quick mode**: Read the proposal from `docs/proposals/test-pipeline-interleaved/proposal.md`. Extract Key Scenarios as Journey candidates. If the proposal lacks `scope` or `success criteria` sections, abort the task with a diagnostic message — Journey generation requires these minimum inputs.
...
5. **Generate Files**: Output one `journey.md` per Journey to `docs/features/test-pipeline-interleaved/testing/<journey-name>/journey.md`
...
- **If AUTO_COMMIT=true**: Skip the user review-and-approval step. After validation passes, directly commit all generated Journey files:
  ```bash
  git add docs/features/test-pipeline-interleaved/testing/
  git commit -m "docs: generate journeys for test-pipeline-interleaved"
  ```
...
- [ ] At least 1 Journey file generated under `docs/features/test-pipeline-interleaved/testing/`
