---
id: "fix-1"
title: "Fix: task 5 leftover Scope references in tests"
priority: "P0"
...
# Fix: task 5 leftover Scope references in tests

## Root Cause

Task 5 removed BodyContext.Scope but left references in extract_test.go (lines 378-431) and created malformed autogen_test_new_section.go (missing package declaration)

## Reference Files

- Source: forge-cli/pkg/task/extract_test.go,forge-cli/pkg/task/autogen_test_new_section.go
- Test script: go build ./...
- Test results: compiler errors: ctx.Scope undefined, syntax error in autogen_test_new_section.go
...
1. Parse `forge-cli/pkg/task/extract_test.go,forge-cli/pkg/task/autogen_test_new_section.go` to extract the first file path (comma-separated).
