---
id: "fix-1"
title: "Fix: update test files for TestCommand removal"
priority: "P0"
...
# Fix: update test files for TestCommand removal

## Root Cause

Task 3 removed TestCommand from structs and changed runUnitTestStep signature but did not update test files: types_test.go, journey_isolation_test.go, quality_gate_test.go

## Reference Files

- Source: forge-cli/pkg/task/types_test.go,forge-cli/internal/cmd/journey_isolation_test.go,forge-cli/internal/cmd/quality_gate_test.go
- Test script: go build ./...
- Test results: compiler errors: TestCommand field references and wrong argument counts in test files
