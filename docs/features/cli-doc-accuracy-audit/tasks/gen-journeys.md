...
Generate test Journey documents for the cli-doc-accuracy-audit feature.
Mode: quick
...
- **Breakdown mode**: Read PRD user stories from `docs/features/cli-doc-accuracy-audit/prd/prd-user-stories.md` and functional specs from `docs/features/cli-doc-accuracy-audit/prd/prd-spec.md`. These are the primary input sources.
- **Quick mode**: Read the proposal from `docs/proposals/cli-doc-accuracy-audit/proposal.md`. Extract Key Scenarios as Journey candidates. If the proposal lacks `scope` or `success criteria` sections, abort the task with a diagnostic message — Journey generation requires these minimum inputs.
...
5. **Generate Files**: Output one `journey.md` per Journey to `docs/features/cli-doc-accuracy-audit/testing/<journey-name>/journey.md`
...
- **If AUTO_COMMIT=true**: Skip the user review-and-approval step. After validation passes, directly commit all generated Journey files:
  ```bash
  git add docs/features/cli-doc-accuracy-audit/testing/
  git commit -m "docs: generate journeys for cli-doc-accuracy-audit"
  ```
...
- [ ] At least 1 Journey file generated under `docs/features/cli-doc-accuracy-audit/testing/`
