...
Detect spec drift between existing project specs and current code for the cli-doc-accuracy-audit feature.
...
## Acceptance Criteria
- [ ] `git diff --name-only main...HEAD` 已执行，变更文件范围已确定
- [ ] 仅检查 domains 与变更文件重叠的 spec，无遗漏且无多余扫描
- [ ] 发现的 drift 已自动修复并提交，或确认无 drift
