---
id: "fix-1"
title: "Fix: e2e tests forgeBinary path resolution on Windows"
priority: "P0"
...
# Fix: e2e tests forgeBinary path resolution on Windows

## Root Cause

forgeBinary() looks for 'forge' without .exe extension; os.Stat fails on Windows, triggering go build in forge-cli/ which has no Go files in standalone module context
