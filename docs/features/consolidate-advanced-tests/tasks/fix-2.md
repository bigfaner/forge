---
id: "fix-2"
title: "Fix: justfile-integration 7 pre-existing failures"
priority: "P0"
...
# Fix: justfile-integration 7 pre-existing failures

## Root Cause

7 pre-existing test failures in justfile-integration. Failures are identical in original forge-cli/tests/ source. Root cause: SKILL.md structure evolved and justfile custom recipes were removed since tests were written. Fix test assertions to match current SKILL.md and justfile content.

## Reference Files

- Source: tests/justfile-integration/
- Test script: cd tests && go test -tags=cli_functional ./justfile-integration/...
- Test results: 7 tests fail: evolved SKILL.md structure and removed justfile custom recipes
...
1. Parse `tests/justfile-integration/` to extract the first file path (comma-separated).
