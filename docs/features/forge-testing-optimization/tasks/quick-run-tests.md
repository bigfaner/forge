...
- Runs spec files from `tests/e2e/features/forge-testing-optimization/`
- Writes `tests/e2e/features/forge-testing-optimization/results/latest.md` (PASS or FAIL)

## Reference Files

- `tests/e2e/features/forge-testing-optimization/` — Test scripts generated by T-quick-2
- `tests/e2e/features/forge-testing-optimization/results/latest.md` — Output: test results report
- `tests/e2e/config.yaml` — Test environment config (optional for CLI-only projects)

## Acceptance Criteria

- [ ] `tests/e2e/features/forge-testing-optimization/results/latest.md` exists
- [ ] All tests pass (status = PASS in latest.md)

## Implementation Notes

1. Run `/run-e2e-tests` skill
2. Run: `just test-e2e --feature forge-testing-optimization`
3. Read `tests/e2e/features/forge-testing-optimization/results/latest.md` to determine outcome
...
- For each distinct root cause, create a fix task:
  ...
  task add --template fix-task \
           ...
           --var TEST_SCRIPT="tests/e2e/features/forge-testing-optimization/<failing-spec>.spec.ts" \
           --var TEST_RESULTS="tests/e2e/features/forge-testing-optimization/results/latest.md" \
