...
Detect spec drift between existing project specs and current code for the global-doc-code-audit feature.

## Acceptance Criteria
- [ ] Changed files identified via `git diff --name-only main...HEAD`
- [ ] Spec files with overlapping domains verified against current code
- [ ] Drifted specs auto-fixed and committed with [auto-specs] tag, or no drift found
