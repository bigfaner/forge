...
dependencies: ["T-review-doc"]
type: "test.gen-journeys"
surface-key: ""
surface-type: ""
---

Generate test Journey documents for the per-task-surface-scoped-gate feature.
Mode: quick
...
- **Breakdown mode**: Read PRD user stories from `docs/features/per-task-surface-scoped-gate/prd/prd-user-stories.md` and functional specs from `docs/features/per-task-surface-scoped-gate/prd/prd-spec.md`. These are the primary input sources.
- **Quick mode**: Read the proposal from `docs/proposals/per-task-surface-scoped-gate/proposal.md`. Extract Key Scenarios as Journey candidates. If the proposal lacks `scope` or `success criteria` sections, abort the task with a diagnostic message — Journey generation requires these minimum inputs.
...
5. **Generate Files**: Output one `journey.md` per Journey to `docs/features/per-task-surface-scoped-gate/testing/<journey-name>/journey.md`
...
- **If AUTO_COMMIT=true**: Skip the user review-and-approval step. After validation passes, directly commit all generated Journey files:
  ```bash
  git add docs/features/per-task-surface-scoped-gate/testing/
  git commit -m "docs: generate journeys for per-task-surface-scoped-gate"
  ```
...
- [ ] At least 1 Journey file generated under `docs/features/per-task-surface-scoped-gate/testing/`
