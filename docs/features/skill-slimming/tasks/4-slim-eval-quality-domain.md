...
## Description
对评测/质量域的 3 个 skill 进行精简和消歧：eval（200 行）、gen-contracts（186 行）、test-guide（186 行）。按 Splitting Heuristic 规则处理每个文件。
...
- 不改变 skill 的输入/输出契约
- eval 已有 34 个辅助文件（2295 行，experts/、rubrics/ 等）——不要破坏现有结构，只在需要时新增 rules/

## Implementation Notes
- eval 已有丰富的子目录结构，优先检查现有文件是否已承担 rules/ 的角色，避免重复拆分
- gen-contracts 有 6 个辅助文件（240 行），可能只需精简不需要拆分
- test-guide 有 4 个辅助文件（235 行），评估是否需要拆分
