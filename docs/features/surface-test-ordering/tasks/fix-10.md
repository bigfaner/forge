---
id: "fix-10"
title: "Fix: task-type-system config fixtures"
priority: "P0"
...
# Fix: task-type-system config fixtures

## Root Cause

Fix config fixtures and assertions in task-type-system

## Reference Files

- Source: tests/task-type-system/*.go
- Test script: cd tests && go test -tags e2e ./task-type-system/... -count=1
- Test results: TestTC_002_ListTypesShowsDeprecatedImplementation, TestTC_011_QualityGateSkipsForCleanupOnly fail
...
1. Parse `tests/task-type-system/*.go` to extract the first file path (comma-separated).
...
When this task is recorded as completed via `task record`, the source task fix-9 is automatically restored to pending if all its dependencies are completed.
