---
id: "fix-9"
title: "Fix: task-lifecycle config fixtures"
priority: "P0"
...
# Fix: task-lifecycle config fixtures

## Root Cause

Fix config fixtures in task-lifecycle: add surfaces field to test helpers

## Reference Files

- Source: tests/task-lifecycle/*.go
- Test script: cd tests && go test -tags e2e ./task-lifecycle/... -count=1
- Test results: TestTC_024_DefaultQueryShowsFourFieldsUnchanged, TestTC_025_VerboseQueryDisplaysAllTaskFields fail
...
1. Parse `tests/task-lifecycle/*.go` to extract the first file path (comma-separated).
...
When this task is recorded as completed via `task record`, the source task fix-8 is automatically restored to pending if all its dependencies are completed.
