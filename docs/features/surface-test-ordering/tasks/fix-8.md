---
id: "fix-8"
title: "Fix: quality-gate config fixtures"
priority: "P0"
...
# Fix: quality-gate config fixtures

## Root Cause

Fix config fixtures in quality-gate: add surfaces field to test helpers

## Reference Files

- Source: tests/quality-gate/*.go
- Test script: cd tests && go test -tags e2e ./quality-gate/... -count=1
- Test results: TestTC_001_AddFixTaskCreatesStepScopedSourceTaskID fails
...
1. Parse `tests/quality-gate/*.go` to extract the first file path (comma-separated).
...
When this task is recorded as completed via `task record`, the source task fix-7 is automatically restored to pending if all its dependencies are completed.
