---
id: "fix-11"
title: "Fix: automated-test-orchestration run-tests removal"
priority: "P0"
...
# Fix: automated-test-orchestration run-tests removal

## Root Cause

Fix or skip tests referencing removed forge run-tests command

## Reference Files

- Source: tests/automated-test-orchestration/*.go
- Test script: cd tests && go test -tags e2e ./automated-test-orchestration/... -count=1
- Test results: TestTC_034 tests removed forge run-tests command
...
1. Parse `tests/automated-test-orchestration/*.go` to extract the first file path (comma-separated).
...
When this task is recorded as completed via `task record`, the source task fix-10 is automatically restored to pending if all its dependencies are completed.
