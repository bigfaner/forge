...
Detect spec drift between existing project specs and current code for the forge-cli-binary-distribution feature.
...
- [ ] `git diff --name-only main...HEAD` executed to identify changed files
- [ ] Only spec files whose `domains` overlap with changed files are verified
- [ ] Drift detected → auto-fixed and committed with `[auto-specs]` tag
