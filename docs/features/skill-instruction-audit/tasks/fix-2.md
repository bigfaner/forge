---
id: "fix-2"
title: "Fix: pre-existing lint failures block quality gate (types.go stutter warnings)"
priority: "P0"
...
# Fix: pre-existing lint failures block quality gate (types.go stutter warnings)

## Root Cause

forge-cli/pkg/task/types.go has 3 revive stutter warnings (TaskTypeInfo->TypeInfo, TaskIndex->Index, TaskState->State) that cause forge task submit quality gate to panic. These are pre-existing in the unify-enum-constants worktree. Renaming these types will fix the lint and unblock fix-1.

## Reference Files

- Source: forge-cli/pkg/task/types.go
- Test script: just lint
- Test results: 3 revive warnings: TaskTypeInfo, TaskIndex, TaskState stutter
...
1. Parse `forge-cli/pkg/task/types.go` to extract the first file path (comma-separated).
