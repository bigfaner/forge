---
id: "fix-4"
title: "Fix: 3 pre-existing TestAddFixTask_* failures in internal/cmd block quality gate"
priority: "P0"
...
# Fix: 3 pre-existing TestAddFixTask_* failures in internal/cmd block quality gate

## Root Cause

3 pre-existing test failures in quality_gate_test.go block forge task submit quality gate. Tests expect errors when surface inference fails on empty output/no source files/hard failure, but the code successfully adds fix tasks instead of returning errors.

## Reference Files

- Source: forge-cli/internal/cmd/quality_gate_test.go
- Test script: go test -race -run 'TestAddFixTask_EmptyOutput|TestAddFixTask_NoSourceFilesInOutput|TestAddFixTask_SurfaceInferenceHardFailure' ./internal/cmd/...
- Test results: 3 FAIL: TestAddFixTask_EmptyOutput, TestAddFixTask_NoSourceFilesInOutput, TestAddFixTask_SurfaceInferenceHardFailure - tests expect error when surface inference fails but task gets added successfully
...
1. Parse `forge-cli/internal/cmd/quality_gate_test.go` to extract the first file path (comma-separated).
