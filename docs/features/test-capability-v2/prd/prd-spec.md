...
- 深度增强：边界/异常场景自动衍生、风险驱动测试密度、surface 差异化测试策略
...
- **Forge 维护者**：维护 Forge 插件技能，需要清晰的管线架构和可扩展的 surface 类型系统
...
| 事实依据（Fact Alignment） | 0–150 | 区分两类声明：**事实声明**（基于 Fact Table 已知事实，需可追溯到具体 fact_id；未知来源标注 `UNKNOWN`）和**合理推理声明**（LLM 衍生的边界 Outcome 不在 Fact Table 中，但基于 surface 类型 `required_outcomes` 规则衍生，需标注推理依据和 `source: inferred`）。评分标准：事实声明需有 fact_id 对应；推理声明需有 `required_outcomes` 规则支撑 |
| Surface 适配（Surface Fitness） | 0–150 | 是否遵循 surface 类型的 `required_outcomes` 规则和测试策略比例 |
| 一致性（Internal Consistency） | 0–150 | Journey Invariants 是否在所有 Step 中成立，跨 Step 引用是否一致 |
- [ ] 边界/异常场景自动衍生引擎：基于 surface 类型 `required_outcomes` 规则 + 项目 Fact Table 自动生成边界和异常 Outcome
- [ ] 风险驱动测试密度：基于 Journey 的 risk_level 字段差异化衍生 Outcome 数量
- [ ] Contract 测试（集成层）+ Journey 烟测试（E2E 层）生成，按 surface 类型差异化侧重比例
- [ ] surface 差异化：CLI/TUI/WebUI/API 核心支持 + Mobile 尽力而为
- [ ] 可扩展 surface 类型系统：surface 类型配置文件（surface rule 文件，存放于 `skills/gen-journeys/rules/surface-*.md`）定义检测规则、测试策略、环境检测项、必须 Outcome，eval rubric 根据配置动态适配评分
- [ ] 内置 Convention 文件扩充（pytest、JUnit、Rust/cargo test）
- [ ] test-guide 增强：自动扫描项目信号检测测试框架并生成 Convention 草稿
- [ ] gen-test-scripts 适配增强后的合约规范和 surface 差异化
- [ ] Run-to-Learn 机制：骨架测试 → 运行捕获输出 → 丰富 Fact Table → 重新生成
- [ ] surface 特定执行环境就绪检测（CLI/TUI/WebUI/API）
...
- 视觉回归测试
- CI/CD 集成或测试环境管理
- 合约 6 维度模型（schema）修改（注：`required_outcomes` 是按 surface 类型配置的实例数据，不属于 schema 变更）
- 已使用 gen-test-cases 项目的迁移工具
- gen-test-scripts 的编译/lint 执行器核心逻辑变更（注：为 Maestro 新增 YAML 输出格式属于 surface 差异化适配，不涉及编译/lint 逻辑本身）
- 跨 surface 组合编排
- 执行环境自动准备与配置（仅做就绪检测）
- 失败诊断 surface 特定策略
- 测试数据管理 surface 特定策略
...
- **Journey**：描述用户完成某个目标的真实工作流，是测试的主要组织单元。包含 Name（kebab-case）、Risk（High/Medium/Low）、Steps（有序动作序列）、Invariants（跨步骤不变量）。（详见 surface rules）
...
- **Fact Table 访问**：通过 `forge fact` CLI 子命令读写 `.forge/fact-table.json`，保证结构化读写的正确性（避免 LLM 直接操作 JSON 导致的数据损坏）。命令包括 `forge fact list`、`forge fact get <id>`、`forge fact summary`。
...
2. 系统检测项目的 surface 类型（CLI/TUI/WebUI/Mobile/API），检测规则见下表：
...
3. 系统通过 docs/conventions/testing/index.md 索引检查 Convention 文件是否存在；若不存在，test-guide 自动检测框架并生成 Convention 草稿供用户审核
...
10. surface 特定环境就绪检测：验证执行环境是否准备好
...
| 源步骤 | 产出数据 | 消费步骤 | 传递方式 |
|--------|---------|---------|---------|
| SURFACE_DETECT | surface 类型（CLI/TUI/WebUI/Mobile/API） | gen-contracts, gen-test-scripts, ENV_CHECK, eval-journey/eval-contract（Surface 适配维度） | 写入 `.forge/config.yaml` 的 `surface` 字段 |
| TEST_GUIDE | Convention 文件（4 个必需 section） | gen-test-scripts（读取断言风格、测试结构、发现规则） | 写入 `docs/conventions/testing/` 目录，文件名含框架标识 |
| GEN_JOURNEY | Journey 文档（含 risk_level） | gen-contracts（衍生策略依据）, eval-journey（评测对象） | 写入 `docs/features/<slug>/testing/<journey>/journey.md` |
| GEN_CONTRACT | Contract 文档（6 维度 + Outcome） | gen-test-scripts（生成代码依据）, eval-contract（评测对象） | 写入 `docs/features/<slug>/testing/<journey>/contracts/step-<N>-<action>.md` |
...
flowchart TD
    ...
    PRD_CHECK -->|是| SURFACE_DETECT[检测项目 surface 类型<br>CLI/TUI/WebUI/Mobile/API]
    SURFACE_DETECT -->|单一类型| DETECT{Convention 文件存在?}
    SURFACE_DETECT -->|未知/混合类型| SURFACE_FAIL[暂停管线<br>输出检测到的信号<br>用户确认 surface 类型]
    SURFACE_FAIL -->|用户选择类型<br>选择存入 session 缓存| DETECT
    ...
    GEN_CONTRACT[gen-contracts<br>生成合约 + 衍生边界 Outcome] --> SCHEMA_VAL{合约 schema 验证<br>6 维度结构完整性 +<br>Preconditions 互斥性}
    SCHEMA_VAL -->|通过| EVAL_C[eval-contract 评测]
    SCHEMA_VAL -->|失败, 重试 1 次| GEN_CONTRACT
    SCHEMA_VAL -->|重试仍失败| SCHEMA_FAIL[暂停管线<br>输出不符合项明细<br>人工修正]
    ...
    ENV_CHECK[环境就绪检测<br>按 surface 类型差异化检测项<br>CLI: 二进制编译 / TUI: 二进制+stdin pipe / WebUI: dev server / API: 服务+DB] -->|通过| CONFIDENCE
...
**Contract 测试占比计算公式**：`Contract 测试函数 / (Contract 测试函数 + Journey 烟测试函数) × 100%`
...
| 5 | gen-test-scripts | 适配 surface 差异化 | 按 surface 类型差异化生成策略和测试层级 |
...
- Convention 文件直接迁移到新 schema，不保留旧 schema 兼容
...
2. **深度增强**：边界衍生 + 风险驱动 + surface 差异化 + eval-journey/eval-contract + Run-to-Learn + 环境检测 + 置信度评级 — 门禁：高风险测试数 ≥ 低风险 × 1.5，边界 Outcome 无效比例 < 20%
...
**Gold Standard 校准方法**：
- **集合规模**：≥ 5 个人工标注的 Journey/Contract 文档对，覆盖 CLI/API/WebUI 三种 surface 类型（注：新增单个 surface 类型时，需提交至少 1 个人工标注的 gold standard 文档对用于校准，该文档对计入总集合规模）
- **标注流程**：由 2 名以上维护者独立评分，取平均分作为 gold standard 基准
- **校准方式**：eval 技能对 gold standard 集评分，若平均偏差 > 10% 则调整 rubric 权重
- **更新频率**：每次新增 surface 类型或修改 rubric 维度时重新校准
...
| Surface 适配 | 90/150 | 不遵循 surface 策略会产出不合适的测试形态 |
...
| SURFACE_FAIL（未知/混合 surface） | 1 | retryable — 用户确认后可重跑 |
...
| surface 差异化过于复杂 | M | M | 每种 surface 收敛到 1 个策略文件 |
