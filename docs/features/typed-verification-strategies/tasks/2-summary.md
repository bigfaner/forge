...
Generate a structured summary of all completed tasks in Phase 2. This summary is read by Phase 3 tasks to maintain cross-phase consistency.

## Instructions

### Step 1: Read all phase task records

Read each record file from `docs/features/typed-verification-strategies/tasks/records/` whose filename starts with `2.` and does NOT contain `.summary`. Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/typed-verification-strategies/tasks/records/2.*.md`
