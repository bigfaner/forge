---
id: "fix-1"
title: "Fix: pre-existing lint failures block quality gate"
priority: "P0"
...
# Fix: pre-existing lint failures block quality gate

## Root Cause

Pre-existing lint issues in upgrade_test.go (unused parameter args) and submit.go (SA4010 append result) block the quality gate for task 1. The forgelog package itself passes lint cleanly. Fix: rename unused param to _, fix append usage in submit.go.

## Reference Files

- Source: forge-cli/internal/cmd/upgrade_test.go,forge-cli/internal/cmd/task/submit.go
- Test script: cd forge-cli && golangci-lint run ./...
- Test results: upgrade_test.go: unused parameter 'args'; submit.go: SA4010 append result never used
...
1. Parse `forge-cli/internal/cmd/upgrade_test.go,forge-cli/internal/cmd/task/submit.go` to extract the first file path (comma-separated).
