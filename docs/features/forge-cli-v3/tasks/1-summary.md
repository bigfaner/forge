...
Generate a structured summary of all completed tasks in Phase 1 (Base Rename). This summary is read by subsequent phase tasks to maintain cross-phase consistency.

## Instructions

### Step 1: Read all phase task records

Read each record file from `docs/features/forge-cli-v3/tasks/records/` whose filename starts with `1.` and does NOT contain `.summary`. Exclude the phase summary's own prior record if one exists.
...
## Key Decisions
- {{each keyDecision from all records, prefixed with task ID: "1.1: ..."}}
...
- All phase task records: `docs/features/forge-cli-v3/tasks/records/1.*.md`
- Design reference: `docs/features/forge-cli-v3/design/tech-design.md`
