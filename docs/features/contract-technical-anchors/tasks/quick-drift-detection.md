...
Detect spec drift between existing project specs and current code for the contract-technical-anchors feature.
...
## Acceptance Criteria
- [ ] 使用 `git diff --name-only main...HEAD` 精确识别变更文件范围
- [ ] 仅检查 `domains` frontmatter 与变更文件有重叠的 spec 文件
- [ ] 漂移的 spec 文件自动修复并提交
