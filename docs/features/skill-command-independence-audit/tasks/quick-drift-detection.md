...
Detect spec drift between existing project specs and current code for the skill-command-independence-audit feature.
...
## Acceptance Criteria
- [ ] Spec drift detected (or confirmed no drift) for files changed by this feature
- [ ] Auto-fixed specs committed with [auto-specs] tag (if drift found)
