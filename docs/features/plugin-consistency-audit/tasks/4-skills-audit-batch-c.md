---
id: "4"
title: "Skills deep audit - batch C (extract-design-md, gen-contracts, gen-journeys, gen-sitemap, init-justfile, submit-task, test-guide)"
priority: "P1"
estimated_time: "2h"
dependencies: [1]
type: "doc"
...
# 4: Skills deep audit - batch C

## Description

对 7 个 skill（extract-design-md, gen-contracts, gen-journeys, gen-sitemap, init-justfile, submit-task, test-guide）执行 Layer 2（指令一致性）和 Layer 3（时序与流程）深度审计。按 proposal 的"逐组件多轮对话"模式执行。
...
- `plugins/forge/skills/{extract-design-md,gen-contracts,gen-journeys,gen-sitemap,init-justfile,submit-task,test-guide}/`: 7 个 skill 目录 (source: proposal.md#审计协议步骤)
...
| File | Description |
|------|-------------|
| `docs/features/plugin-consistency-audit/reports/04-skills-batch-c.md` | Skills batch C 审计报告（Layer 2-3 发现） |
...
## Hard Rules
- 仅审计以下 7 个 skill 目录：extract-design-md, gen-contracts, gen-journeys, gen-sitemap, init-justfile, submit-task, test-guide。不修改任何文件。

## Implementation Notes
- 冗余检测启发式：同一语义在 ≥2 个文件中出现且无信息增量才标记 REDUNDANT
